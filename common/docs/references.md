...
## Непропорциональность светового выхода

Почему это здесь: модель считает **выделенную энергию**, а прибор регистрирует
**свет**, и связь между ними нелинейна. Практические следствия сведены в
`common/docs/method-rules.md` раздел 7a; подробный разбор основного источника —
`common/docs/nonproportionality-npsm-breitenmoser-2023.md`.

- **Breitenmoser D., Cerutti F., Butterweck G., Kasprzak M.M., Mayer S.**
  ...
  https://doi.org/10.1038/s41467-023-42574-y — открытая (CC BY).
  Главное для нас: параметры модели измерены **для каждого экземпляра
  кристалла** и значимо разошлись между четырьмя кристаллами одной поставки
  (Биркс 254…415 МэВ·см⁻¹) и с табличными значениями. Дополнительные материалы
  (53 стр., оттуда же) содержат `Algorithm S1` — эталонный псевдокод
  взвешивания энерговыделения, включая обработку локальных депозитов, — и
  полуаналитическую модель сдвига края через число комптоновских рассеяний.
  Расчёты авторов на FLUKA, но механизм внедрения переносится на Geant4 прямо.
  Открыты и данные: измерения `10.3929/ethz-b-000528920`, подпрограммы FLUKA
  `10.3929/ethz-b-000595727` (архив требует учётной записи ETH, README открыт).

- **Payne S.A. и др.** Nonproportionality of scintillator detectors, части I–V
  (2009–2015). Части, которыми пользовались: **I** — вывод модели (Онзагер,
  захват, Биркс) и её уравнения; **II** — параметры по материалам;
  **III** — температурная зависимость, *IEEE TNS* **61**(5):2771 (2014),
  DOI 10.1109/TNS.2014.2343572 (именно её реализуют подпрограммы выше);
  **IV** — дельта-электроны и вклад в разрешение.

- **Khodyuk I.V., Dorenbos P.** Trends and patterns of scintillator
  nonproportionality. arXiv:1204.4350 (2012). Обзорная рамка: как ведут себя
  разные кристаллы и почему разброс между образцами не шум.
