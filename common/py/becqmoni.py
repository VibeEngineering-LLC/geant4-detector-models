...
def broaden(hist, fwhm_at_662=49.9, emax=None, bin_keV=1.0, fwhm_of=None):
    ...
    """
    # ПОТОЛОК ВЫВОДИТСЯ ИЗ ДАННЫХ, а не задан числом. Прежде стояло
    # emax=3200 при сетке модели до 3700 кэВ, и это была МОЛЧАЛИВАЯ ветвь: на
    # E0 = 3552,5 условие `hi <= lo` срабатывало и пик выбрасывался целиком —
    # `area_broadened` вернула бы 0, а потребитель отбросил бы точку без
    # всякого признака. Узлы 3304,8 и 3552,5 существуют в сетке, и хотя
    # размытый путь их сегодня не зовёт (сетки идут депозитным окном, а линии
    # комплекта не выше 2614,5), латентная ветвь остаётся. Указание аудитора:
    # закрыть навсегда, а не подпереть сторожем.
    #
    # Запас 8σ на верхнем краю: 5σ хватает самому пику, остальное — чтобы
    # нормировка `g/s` не обрезалась у последнего канала.
    top = max([e for e, c in hist.items() if c > 0], default=bin_keV)
    sig_top = (fwhm_of(max(top, 8.0)) if fwhm_of is not None
               else fwhm_at_662 * math.sqrt(max(top, 8.0) / 661.657)) / 2.3548
    need = top + 8 * sig_top
    if emax is None:
        emax = need
    elif need > emax:
        raise SystemExit(
            "becqmoni.broaden: потолок emax = %.0f кэВ мал для спектра с "
            "каналами до %.1f кэВ (нужно %.0f).\n"
            "Прежде такой пик выбрасывался МОЛЧА и площадь выходила нулевой. "
            "Уберите emax (он выводится из данных) либо задайте не меньше "
            "нужного." % (emax, top, need))
