...
def passport(detector="Gamma-1S"):
    """Файл с активностями/датами поверки источников комплекта — ЛИЧНЫЙ.

    В отличие от `ref()`, у этого файла НЕТ закоммиченного варианта, даже
    урезанного: активности и точные даты поверки 25 источников вместе — это
    квазиидентификатор конкретного купленного комплекта (решение оператора
    31.07.2026, задача 133 п.(в)). Только G4MODELS_PASSPORT_<ДЕТЕКТОР> или
    G4MODELS_PASSPORT; без них — None, и вызывающий обязан сообщить, откуда
    взять файл, а не тихо посчитать с пустым паспортом.
    """
    return (_env("G4MODELS_PASSPORT_" + _slug(detector))
            or _env("G4MODELS_PASSPORT"))
