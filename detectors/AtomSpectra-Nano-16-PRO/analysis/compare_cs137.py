# -*- coding: utf-8 -*-
"""Опорная сверка: модель Geant4 против собственной записи Cs-137, 10 см от торца.
...
def main():
    ...
    # --- измерение -----------------------------------------------------------
    k = sam.live / bg.live                      # масштаб фона по ЖИВОМУ времени
    s_win, nch = sam.counts_between(*WIN)
    b_win, _ = bg.counts_between(*WIN)
    net_win = s_win - k * b_win
    d_net_win = math.sqrt(s_win + k * k * b_win)

    # Подгонка пика. Спектр приводится к отсчётам НА КЭВ: шаг канала на этом
    # приборе не постоянен (калибровка 4-й степени), и без деления на ширину
    # канала площадь под гауссианой не была бы числом отсчётов.
    ...
    print("  живое время образца %.0f с, фона %.0f с, масштаб фона %.5f"
          % (sam.live, bg.live, k))
    ...
    for name, ne, dn, em, ee in (
            ...
            ("B) ROI +-1,25 ПШПВ", net_roi, d_net_roi, m_roi, eps_m_roi)):
        eps_e = ne / denom
        print("%-34s %12.4e %12.4e %8.3f"
              % (name + ", eps(4pi)", eps_e, ee, ee / eps_e))
        print("%-34s %8.0f +- %-3.0f %12.0f" % ("   отсчётов", ne, dn, em))
    print()
    print("статистика: измерение %.1f / %.1f %% (окно / ROI), модель %.2f %%"
