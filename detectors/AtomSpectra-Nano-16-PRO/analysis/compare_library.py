...
Сетки разные (наша 29 узлов, библиотечная 34). Узел считается ОБЩИМ только при
точном совпадении энергий; иначе библиотечная берётся лог-лог интерполяцией её
же кривой к НАШЕЙ энергии, и такая точка помечается — интерполяция вносит свою
погрешность, а объявить её нечем.
...
# Допуск «тот же узел». Был 1,0 кэВ — при нём наш узел 59,541 сравнивался с
# библиотечным 60,0 НАПРЯМУЮ, без поправки на 0,46 кэВ разницы. На мягком крае
# кривая растёт круто, и это занижало расхождение на 0,43 п.п. (+8,49 % вместо
# +8,92 %) — больше нашей статистики на узле (0,24 %) и сравнимо с объявленной
# погрешностью библиотеки (0,71 %). Найдено слепым состязательным аудитом
# 06.08.2026. Теперь общим считается только точное совпадение; 0,05 кэВ
# оставлено на представление чисел, а не на физику.
E_TOL_KEV = 0.05
