...
классе. Плотности взяты из базы Geant4/NIST, и суммарная поверхностная
плотность рабочей грани сверяется с величиной, которую печатает сама программа
при каждом запуске: расхождение выше 0,1 % останавливает скрипт.

Показаны ДВА стека, потому что они разные, а в пучке стоит второй:
  рабочая грань (Y) — стенка корпуса Al 1,20;
  торец (Z)        — крышка ABS 2,00 и воздушный зазор, стенки Al нет.
...
# Поверхностная плотность лицевого стека, которую печатает сама программа
# (ReportMasses). Служит замком: если разбор .hh или плотности разъедутся,
# скрипт остановится, а не нарисует красивую неправду.
FACE_STACK_PRINTED = 0.570870      # г/см²
...
RHO = {"CsI(Tl)": 4.510, "ПТФЭ": 2.200, "Al": 2.699, "ABS*": 1.060}
...
def geom_from_header(path):
    ...
    need = ("ptfe", "alFoil", "wFront", "wCap", "cryY", "cryZ")
...
def main():
    ...
    face = [("ПТФЭ (на кристалле)", g["ptfe"], RHO["ПТФЭ"], "#f2f2f2"),
           ...
           ("Al-фольга (на ПТФЭ)", g["alFoil"], RHO["Al"], "#c9c9c9"),
           ("воздух полости", 0.90, None, "#eaf1f7"),
           ("крышка ABS*", g["wCap"], RHO["ABS*"], "#5c5f63")]

    fig, (a1, a2) = plt.subplots(1, 2, figsize=(11.6, 6.4))
    t_face = draw_stack(
        a1, face, "Рабочая грань 18 × 57 мм (ось Y)",
        "в настоящей постановке НЕ в пучке")
    t_end = draw_stack(
        a2, end, "Торец 18 × 15 мм (ось Z)",
        "В ПУЧКЕ: источник обращён сюда")

    if abs(t_face - FACE_STACK_PRINTED) / FACE_STACK_PRINTED > 1e-3:
        raise SystemExit(
            "стек рабочей грани %s г/см² не сходится с печатью программы %s — "
            "разошлись разбор .hh или плотности" % (ru(t_face, 6),
                                                    ru(FACE_STACK_PRINTED, 6)))
    ...
    fig.text(0.5, 0.035,
             ...
             "* вещества-заменители: корпус и фольга — чистый Al при "
             "заявленном сплаве, крышка — полистирол вместо ABS. Воздушный "
             "зазор торца показан условной толщиной, в модели он переменный.",
             fontsize=8, ha="center", color="#555555")
    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    fig.savefig(OUT, dpi=160, bbox_inches="tight")
    print("записано: %s" % OUT)
    print("стек рабочей грани %s г/см² (программа печатает %s)"
          % (ru(t_face, 4), ru(FACE_STACK_PRINTED, 4)))
