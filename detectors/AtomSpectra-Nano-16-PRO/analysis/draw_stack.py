...
`FACE_STACK_PRINTED`, переписанной из расчёта с шестью знаками. Это сверка
`.hh` с этой константой, а НЕ с печатью программы: программа печатает `%.3f`,
то есть 0,571, и до шестого знака сверять с ней нечего. Торцевой стек не
сверяется ни с чем — независимого числа для него нигде не печатается.

Показаны ДВА стека, потому что они разные, а в пучке стоит второй:
  рабочая грань (Y) — стенка корпуса Al 1,20;
  торец (Z)        — крышка 2,00, стенки Al нет; воздушного зазора между
                     обёрткой и крышкой НЕТ (`ASN16Detector.cc:137`:
                     `zCapFi = zFoilF`, обёртка упёрта в крышку).
...
def main():
    ...
    fig.text(0.5, 0.035,
             ...
             "заявленном сплаве, крышка — полистирол вместо ABS. Воздушного "
             "зазора между обёрткой и крышкой в модели нет: обёртка упёрта в "
             "крышку (ASN16Detector.cc:137).",
