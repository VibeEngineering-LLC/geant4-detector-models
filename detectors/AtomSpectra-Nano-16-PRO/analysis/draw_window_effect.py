...
Библиотечная кривая наложена третьей: она отвечает на вопрос, какую из двух
воспроизводит модель автора библиотеки.
...
def main():
    ...
    ax.plot([r[0] for r in lib], [r[1] for r in lib], "^:", ms=4.6, lw=1.3,
            color=C_LIB, label="библиотечная BecqMoni (у автора Al 2,00 "
                               "сплошной)")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    fig.text(0.5, 0.012,
             "Библиотечная кривая ложится на расчёт БЕЗ фрезеровки "
             "(+2…+4 % на мягком крае — остаток от разницы 1,50 и 2,00 мм), а "
             "не на расчёт с ней (+18 % на 40 кэВ):\n"
             "модель автора библиотеки выборку не учитывает. Жёсткий край "
             "расходится одинаково в обоих случаях, то есть к окну отношения "
             "не имеет.",
