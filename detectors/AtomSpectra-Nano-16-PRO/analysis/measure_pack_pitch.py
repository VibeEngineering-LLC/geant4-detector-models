...
ОТКАЗ ЭТОГО МЕТОДА, ЗАФИКСИРОВАННЫЙ 06.08.2026. Шаг, полученный здесь (6,15 мм),
ОТМЕНЁН замером оператора (8,1 мм). Период у обоих замеров совпал — 52 против
53,3 px, — разошлась ШИРИНА СТЕРЖНЯ: порог «доля отсчётов ниже среднего»
захватывает затенённые рёбра лотка вместе с прутком и завышает ширину примерно
в 1,7 раза. Отношение шаг/диаметр вышло 1,46–1,92 вместо 2,55.

Отсюда правило: период по автокорреляции этот метод считает надёжно, а ШИРИНУ
надо брать сегментацией краёв прутка на профиле яркости (у оператора —
20,8–21,0 px при трёх независимых сканлиниях, разброс результата 8,11 / 8,21 /
8,21 мм). Порог по средней яркости для ширины не годится.
...
поля, поэтому участок берётся коротким. Замер линейкой отменяет любой замер по
фотографии.
