...
# Содержание ThO2 в электродах марки WT20 (WTh-20): по спецификации марки
# 1,70…2,20 % масс. (сообщено оператором 07.08.2026, сменяет прежнее
# допущение «ровно 2 % по этикетке» точкой ряда — диапазон, а не число).
# Все активности ниже линейны по этой доле (см. `activities_after_purification`),
# поэтому диапазон переносится в результат простым масштабом, без пересчёта
# каждой таблицы заново.
THO2_FRAC_LO = 0.0170
THO2_FRAC_HI = 0.0220
THO2_FRAC_MID = 0.5 * (THO2_FRAC_LO + THO2_FRAC_HI)
TH_IN_THO2 = 0.878809          # массовая доля Th в стехиометрическом ThO2
                                # (232,038 / 264,037 г/моль, см. §11 приложения)


def th232_activity(mass_g, w_tho2=THO2_FRAC_MID):
    """Активность Th-232 в пачке, Бк, из массы и доли ThO2 по массе."""
    th_g = mass_g * w_tho2 * TH_IN_THO2
    return th_g * SPEC_ACT_TH232
...
def activities_after_purification(t_sec, a0, hl, br_tl):
    ...
    # Периоды: Ra-224 3,63 сут, Rn-220 55,6 с, Po-216 144,0 мс, Pb-212 10,6 ч,
    # Bi-212 60,6 мин, Tl-208 3,05 мин, Po-212 294,4 нс — все много меньше
    # периода Th-228 (1,91 года). Для такой пары решение цепочки за время,
    # много большее периода дочернего, выходит на ПОДВИЖНОЕ равновесие, но
    # множитель превышения — ПРОИЗВЕДЕНИЕ по цепочке, а не отношение одного
    # звена. Проверка для двух шагов (0 -> 1 -> 2, N₁(0)=N₂(0)=0), асимптотика
    # Бейтмана при λ₀ ≪ λ₁, λ₂ (выживает только член e^{-λ₀t}):
    #
    #     A₂(t) -> A₀(t) · [λ₁/(λ₁−λ₀)] · [λ₂/(λ₂−λ₀)],
    #
    # то есть множитель звена 2 несёт в себе и множитель звена 1 — избыток не
    # начинается заново на каждом шаге, а накапливается. Для нашей цепочки:
    #
    #     A_k = A(Th-228) · Π_{j=1}^{k} λ_j / (λ_j − λ_Th228),
    #
    # произведение по ВСЕМ промежуточным звеньям от Ra-224 до k включительно.
    # Прежняя редакция применяла множитель ТОЛЬКО звена k (без множителей
    # предыдущих), что для Pb-212 и всего, что ниже, теряло вклад Ra-224 —
    # самого медленного звена, дающего наибольший избыток. Расхождение мало
    # (доли процента: избыток Ra-224 — 0,52 %, у Pb-212 собственного звена —
    # 0,063 %; накопленный для Pb-212, Bi-212, Tl-208, Po-212 — около 0,58 %),
    # но заявленная точность («величина верна, а не «примерно верна»») этого
    # не допускала (найдено внешним аудитом, 07.08.2026).
    #
...
def load_templates(tdir, centres, fwhm_fn):
    ...
    Ревизия шаблонов проверяется по `src_sha1` шапки: расхождение между
    файлами одного набора означает смешение разных прогонов Geant4, и
    результат такого смешения молча неверен (в `wt20_unfold.py` эта же
    проверка есть, здесь её не было — задача №79/Ж5 внешнего аудита).
    """
    out, head0 = {}, {}
    sha = None
    for key, lab, _el, _fn, _br in CHAIN:
        ...
        head0 = head0 or head
        h_sha = head.get("src_sha1")
        if h_sha:
            if sha is None:
                sha = h_sha
            elif h_sha != sha:
                raise SystemExit(
                    "шаблоны из разных ревизий исходников: %s имеет "
                    "src_sha1=%s, ожидалось %s" % (p, h_sha, sha))
...
def load_raw_templates(tdir):
    """Шаблоны БЕЗ уширения — (E, счёт, N_primaries) по нуклиду.

    `load_templates` уширяет и нормирует, теряя доступ к исходным парам
    (E, счёт): для честной дисперсии СУММЫ по окну (не суммы дисперсий по
    каналу — соседние каналы коррелированы одним и тем же МК-розыгрышем,
    размытым ядром формы линии на несколько каналов сразу) нужны именно они.
    См. `wt20_age.window_net_var`.
    """
    out = {}
    for key, lab, _el, _fn, _br in CHAIN:
        p = os.path.join(tdir, "%s.csv" % key)
        if not os.path.exists(p):
            continue
        head, e, c = read_template(p)
        n = float(head.get("N_primaries", "0"))
        if n <= 0 or c.sum() <= 0:
            continue
        out[key] = (e, c, n)
    return out
...
def main():
    ...
    a0_lo = th232_activity(mass_g, THO2_FRAC_LO)
    a0_hi = th232_activity(mass_g, THO2_FRAC_HI)
    print("\nсостав по спецификации марки WT20 (WTh-20): ThO2 %.2f...%.2f %% "
          "масс. (среднее %.2f %%), пачка %.1f г"
          % (100 * THO2_FRAC_LO, 100 * THO2_FRAC_HI, 100 * THO2_FRAC_MID,
             mass_g))
    print("активность Th-232: %.0f Бк по среднему содержанию; "
          "%.0f...%.0f Бк по всей вилке марки"
          % (a0, a0_lo, a0_hi))
    print("[таблицы и спектры ниже считаны по СРЕДНЕМУ; все активности линейны "
          "по a0, поэтому масштаб на границы вилки: x%.4f и x%.4f]"
          % (a0_lo / a0, a0_hi / a0))
    ...
    with io.open(p_act, "w", encoding="utf-8", newline="") as f:
        ...
        w.writerow(["# A(Th-232) = %.0f Бк при среднем содержании ThO2 "
                    "%.2f %% масс. (спецификация марки WT20/WTh-20: "
                    "%.2f...%.2f %%, отсюда A(Th-232) = %.0f...%.0f Бк) "
                    "и массе пачки %.1f г; активности ниже линейны по "
                    "содержанию, масштаб на границы вилки x%.4f/x%.4f"
                    % (a0, 100 * THO2_FRAC_MID, 100 * THO2_FRAC_LO,
                       100 * THO2_FRAC_HI, a0_lo, a0_hi, mass_g,
                       a0_lo / a0, a0_hi / a0)])
