...
# Кривая ПШПВ(E) прибора — единственный источник ширины аппаратной линии.
# Ставится в main() из файла замера (узел SimpleSqrtFwhmCalibration) и под
# измеренный спектр НЕ настраивается (директива оператора 07.08.2026:
# «никакой подгонки ПШПВ», «выводи как получилось, под спектр не подгонять»).
WIDTH_FN = None
...
def main():
    ...
    # Единый источник ширины аппаратной линии на весь расчёт. Задаётся ОДИН
    # раз кривой прибора и дальше не меняется: свёртка шаблонов, дисперсия
    # шаблонов и ширины окон промера берут ширину отсюда.
    global WIDTH_FN
    WIDTH_FN = fwc0
    if fwc0 is not None:
    ...
    def gfull(gname, f0, f1):
        ...
        if key not in _gcache:
            ...
            for nk, wgt_k in GROUP[gname].items():
                if nk in tmap:
                    e, c = tmap[nk]
                    # Ширина — из КРИВОЙ ПРИБОРА (WIDTH_FN, узел
                    # SimpleSqrtFwhmCalibration файла замера), поточечно для
                    # каждой линии шаблона. Двучлен (f0, f1) в свёртке не
                    # участвует и остаётся только ключом кэша и подписью для
                    # вывода. Подгонять ширину под спектр запрещено
                    # (директива оператора 07.08.2026).
    ...
    if GLOBAL_FIT:
        # --- ширина аппаратной линии: НЕ ПОДГОНЯЕТСЯ ------------------------
        # Директива оператора 07.08.2026: «никакой подгонки ПШПВ», «выводи как
        # получилось, под спектр не подгонять».
        #
        # Ширина аппаратной линии есть свойство ПРИБОРА и в этом расчёте —
        # входная величина. Берётся кривая ПШПВ(E), записанная самим прибором
        # в файл замера (узел SimpleSqrtFwhmCalibration, восемь опорных точек),
        # и применяется к шаблонам как есть. Ни один её параметр под измеренный
        # спектр не настраивается.
        #
        # Отменены обе прежние схемы, обе были подгонкой под спектр:
        #   — перебор пары (f₀, f₁) двучлена ПШПВ² = f₀ + f₁·E по сетке вместе
        #     с активностями, критерий χ² всей подгонки. Ширина переставала
        #     быть разрешением прибора и вбирала всякое расхождение формы:
        #     избыток модельного континуума компенсировался уширением линий,
        #     и подобранная ПШПВ уходила к 94 кэВ на 2614,51;
        #   — подгонка множителя и наклона поверх кривой прибора
        #     (ПШПВ = k·ПШПВ_файла·(E/662)^p) — то же самое, только в двух
        #     параметрах вместо двух других.
        #
        # Расхождение кривой прибора с прямым промером пиков этого спектра
        # (92,4 против 70,4 кэВ на 2614,51) НЕ устраняется правкой ширины: это
        # факт о приборе или о замере, и он выводится как есть, вместе с
        # последствиями для сверки по пикам. Промер печатается рядом, чтобы
        # расхождение было видно числом.
        if fwc0 is None:
            raise SystemExit(
                "в файле замера нет узла SimpleSqrtFwhmCalibration: ширину "
                "аппаратной линии взять неоткуда, а подгонять её под спектр "
                "запрещено (директива оператора 07.08.2026)")
        fwhm_fn = fwc0
        # Двучлен ПШПВ² = f₀ + f₁·E остаётся только как ПОДПИСЬ для вывода и
        # для тех мест кода, где ширина нужна числом (промер пиков, ширина
        # окон). Он аппроксимирует кривую прибора на рабочей полосе, а не
        # подгоняется под спектр.
        Ew = np.linspace(E_FIT[0], E_FIT[1], 60)
        p_fw, k_fw = np.polyfit(Ew, np.asarray(fwc0(Ew), float) ** 2, 1)

        print("\n--- ширина аппаратной линии: кривая прибора, без подгонки ---")
        print("  источник: узел SimpleSqrtFwhmCalibration файла замера")
        width_rows = []
        for e0 in W_ANCHORS:
            f_dev = float(fwc0(e0))
            f_obs = measure_fwhm(resid_full, e0, f_dev)
            width_rows.append((e0, f_obs if f_obs else float("nan"), f_dev))
            if f_obs and math.isfinite(f_obs):
                print("  %8.2f кэВ: прибор %6.2f кэВ, промер по спектру %6.2f,"
                      " расхождение %+5.1f %%"
                      % (e0, f_dev, f_obs, 100.0 * (f_dev - f_obs) / f_obs))
            else:
                print("  %8.2f кэВ: прибор %6.2f кэВ, промер не удался"
                      % (e0, f_dev))
        print("  аппроксимация кривой на полосе: ПШПВ² ≈ %.0f + %.3f·E"
              " (для вывода, в свёртке не участвует)" % (k_fw, p_fw))

        # --- разложение при заданной ширине ---------------------------------
        # Свободных параметров формы нет: подгоняются ТОЛЬКО активности.
        # Решение — взвешенный МНК, нормальная система D·A = b с итерационными
        # весами, включающими дисперсию шаблонов, и σ(A) из диагонали D⁻¹.
        ...
        A = build(k_fw, p_fw)
        VarA = build_var(k_fw, p_fw)
        x, x_sigma, chi2, x_active = lsrm_solve(A, VarA, yy, ybg[m])
        print("\n[разложение по окну %.0f-%.0f кэВ, ширина задана прибором, "
              "подгоняются только активности]" % E_FIT)
    else:
