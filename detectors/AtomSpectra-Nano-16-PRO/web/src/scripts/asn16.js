...
cvComp.addEventListener('pointermove', function (ev) {
  ...
  roComp.style.left = Math.min(Math.max(4, x + 16), compGeom.W - bw - 4) + 'px';
  roComp.style.top = (y + bh + 18 > compGeom.H ? Math.max(4, y - bh - 12)
    : y + 16) + 'px';
});
...
cv.addEventListener('pointermove', function (ev) {
  ...
  ro.style.left = Math.min(Math.max(4, x + 18), geom.W - bw - 6) + 'px';
  ro.style.top = (y + bh + 20 > geom.H ? Math.max(4, y - bh - 14) : y + 18) + 'px';
