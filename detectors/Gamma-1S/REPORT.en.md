...
> **THE VOLUME-GEOMETRY NUMBERS OF THIS SECTION WERE RETRACTED ON 2026-07-31.** They were taken from a `results/` export made on 28.07 at 22:45, while the calculation grids were recomputed after that on the corrected geometry (commits `2df1eb2` — wall/bottom overlap in the flat cups, `327c5f5` — removal of the side rubber wrap of the case). The export was not refreshed and carried no provenance stamp, so the discrepancy was never flagged (task 137). Recomputation on the current grids (fingerprint `bd17f9bea1e4`) gives for the Marinelli **1.2526 ± 0.0120** instead of 1.165 ± 0.011, shape RMS 10.4% instead of 11.3%, χ²/dof 8.37 instead of 9.8. Efficiency shift across the volume grids: Marinelli and water — median +8%, Petri and Denta cups — median +15…19% (up to +28% on individual nodes). Point geometries are unaffected: median +0.3%, no node beyond 3σ. The ORIGINAL text is left below as written; read it with this correction, and take current numbers from `results/`. The comparison with RadiaCode-103 in the next paragraph is affected too: the K_NORM equivalent became 0.798 instead of 0.858, against 0.833 for the RadiaCode, so the agreement of the two instruments is not 3% but 4.2%. The qualitative conclusion of that paragraph (the systematic belongs to the setup, not to the model of a specific instrument) is not overturned, but it no longer rests on a 3% agreement.
...
**Weighted-average MC/experiment = 1.094 over 24 points** at 5 cm and **1.010 over 20 points** at 25 cm (2026-07-28, ρ(MgO) = 0.8 g/cm³ — entrance-face and reflector fix, see §5a and `common/docs/references.md`).

| geometry | MC/exp | points | shape RMS | χ²/dof |
|---|---|---|---|---|
| point 5 cm, lid CLOSED | 1.094 | 24 | 7.7% | 6.52 |
| point 25 cm, lid OPEN | 1.010 | 20 | 4.0% | 2.09 |
| Marinelli 1 L | 1.165 | 15 | 11.3% | 9.8 |

**The conclusion drawn below in this section is RETRACTED by the geometry fix — it is restored here as originally written, for honesty's sake, with an explanation of what changed.** Before the entrance-face and reflector-density fix (as recorded on 2026-07-27): the point at 5 cm gave 0.971, the point at 25 cm — 0.928, both close to unity and agreeing with each other within 4%, whereas the Marinelli result was off by 20 percentage points — from which it was concluded that the discrepancy belonged to the vessel geometry, not the detector.

After the fix, that support is gone. The point at 5 cm rose to 1.094 — noticeably above unity and noticeably CLOSER to the Marinelli value (1.165) than it was; the gap between the two point geometries grew from 4% to 8%. The reason is visible from the reflector-density scan (§5a): the change in ρ(MgO) raised not only the entrance face (6 mm, which governs the soft edge) but also the radial layer (3.65 mm, which covers the whole crystal) — the two effects are entangled in one parameter, and the hard edge of the point curve crept upward together with the soft edge. The tabular logic "point ≈ unity, therefore the detector is clean" no longer holds after this fix. What can be said RELIABLY: the Marinelli result is still above both point geometries (1.165 against 1.094 and 1.010), i.e., some contribution from the vessel's volume geometry probably remains — but it can no longer be isolated quantitatively by simple subtraction, since the reference point (point ≈ 1) has itself shifted. Section 2.2 (energy deposition versus peak area) remains an open question, neither retracted nor confirmed.
...
**The EffCalcMC point-source calculation (5 cm, 10⁸ trials) closed the fork for good.** On the geometry where the energy dependence is MEASURED, the two codes agree within an RMS of 2.9% (all 22 nodes: mean 0.968, slope −0.4% per decade; soft nodes 45–60 keV — 1.017…1.024), whereas the measured curve departs from both by 0.78 → 1.14 across the range. Along the way, the soft structure of the Marinelli comparison was explained: the +20% at 45–60 keV in the point-source case disappeared — it belonged to the fill (self-absorption in OISN-16), not to the detector. The inter-code residual of +5.6%/decade above 90 keV (+7.5 in the Marinelli case) is probably cross-sections; it is 6–8 times smaller than the effect being sought. Files — `nuclidemaster/EffReg_G1S_Point5cm.{efa,efr}`. **ATTENTION: these files are STALE** — computed by EffCalcMC against a `.din` exported BEFORE the entrance-face and reflector fix (2026-07-28); the operator's fresh `.din` has not been re-exported, and the code comparison below pertains to the earlier geometry and has not been recomputed.

**The explanation of the 0.968 level via a difference in distances is WRONG and has been retracted** (caught by the auditor: passport item 2.10 sets the distance "from the surface of the detector lid," while `.sin` has `Distance,cm=5` with no compensation; the argument rested on a rubber protector which, after the geometry fix (see below), does not exist).

**The entrance-face column has been CHECKED on the new grid, and the outcome is mixed.** The reflector density was reconsidered: the physical range for a loose powder is 0.5–1.2 g/cm³ (not 1.5–2.4 as previously assumed — the old scan sat above the physical range), and ρ = 0.8 was adopted without fitting to the lines (justification and sources — `common/docs/references.md`). A direct Geant4 scan at 4 lines (59.5/88.0/121.8/661.7 keV, normalized to 661.7) gave the right direction but the wrong magnitude and shape: 59.5 keV moved close to the predicted point (MC/exp 0.899 against a predicted 0.913), but 88.0 and 121.8 keV, previously close to unity, OVERSHOT upward (1.02 and 1.05), and the hard edge, which was not supposed to be involved, did not stay put as expected — it rose together with the soft edge. The reason: the radial MgO layer (3.65 mm, covering the whole crystal) responds to the same density as the entrance-face layer (6 mm) — one parameter governs two different geometric locations at once. The full grid at ρ = 0.8 (see the table above in §2.4a): the p5cm average is 1.094 (was 0.971 at ρ = 2.0), the p25cm average is 1.010 (was 0.928). The soft-edge deficit at the softest line genuinely shrank (59.5 keV: 0.831 → 0.933 in absolute MC/exp), but at the cost of a systematic upward shift of the rest of the curve. Open question: split the reflector density into separate radial and entrance-face values (the drawing does not forbid this — the packing could have settled unevenly), or look for the cause of the hard-end excess elsewhere — in the crystal dimensions, as the auditor suspected from the start.
...
The finished computed curves are in [results/](results/), with their description and caveats in [results/README.en.md](results/README.en.md).
