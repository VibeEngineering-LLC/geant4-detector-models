...
def main():
    ...
    hard = [(E, B) for E, _, _, B, _, _ in table if E >= 583.0]
    soft = [(E, B) for E, _, _, B, _, _ in table if E < 583.0]
    print("\nФОРМА B(E).")
    if len(hard) >= 2:
        Es = np.array([E for E, _ in hard])
        Bs = np.array([B for _, B in hard])
        sl, ic = np.polyfit(np.log10(Es), Bs, 1)
        print("  по решающим линиям (E >= 583 кэВ; %d шт.): наклон"
              " %+.4f на декаду; B(662) = %.4f; B(2614) = %.4f"
              % (len(hard), sl, ic + sl * math.log10(661.657),
                 ic + sl * math.log10(2614.511)))
        print("  перепад по этому диапазону: %+.1f %%"
              % (100 * (Bs[-1] - Bs[0]) / Bs[0]))
        if abs(100 * (Bs[-1] - Bs[0]) / Bs[0]) < 2.0:
            print("  ВЫВОД: поправка конвенции ПЛОСКАЯ — она снимает"
                  " ПЛАТО расхождения; разрыв на жёстком крае остаётся.")
        else:
            print("  ВЫВОД: поправка конвенции РАСТЁТ по модулю с энергией —"
                  " часть разрыва на жёстком крае принадлежит ей.")
    if soft:
