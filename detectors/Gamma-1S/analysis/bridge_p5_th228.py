...
ЧИСЛА ЭТОГО ФАЙЛА ОТОЗВАНЫ (30.07.2026): B(662) и B(2614) считались на
моно-спектрах от 28.07, предшествующих правке геометрии торца (коммит
1a29dee); прямое A/B на 122,1 кэВ даёт +17,8 %. Мост стоял на смеси двух
геометрий. Пересчёт на свежих 4pi-моно — задача 115. До пересчёта числа
отсюда не цитировать.
...
# 860,557 добавлена прогоном 30.07.2026 (scat_mono_81_861.mac) — это
# интермедиатная точка, закрывающая вопрос о колене B(E) между 662 и 2614.
MONO = {661.657: ("scat_p5_full_E661.7.csv", 0.00035),
        860.557: ("scat_p5_full_E860.6.csv", 0.00055),
        2614.511: ("scat_p5_full_E2614.5.csv", 0.00139)}
# Отношение eps модель/штатная из hard_edge (метод Base) — цель снятия.
# На 861 линия плато есть, на 662 линии Th-228 нет.
EXCESS = {661.657: None, 860.557: 0.0886, 2614.511: 0.2750}
...
if __name__ == "__main__":
    ...
    b = {r[0]: r[3] for r in rows}
    b662, b861, b2614 = b[661.657], b[860.557], b[2614.511]
    print("\nФОРМА B(E) на трёх опорных точках:")
    print("  662 -> %.4f   861 -> %.4f   2614 -> %.4f"
          % (b662, b861, b2614))
    lin = b662 + (b2614 - b662) * (860.557 - 661.657) / (2614.511 - 661.657)
    print("  линейная интерполяция 662->2614 в точке 861 дала бы %.4f;"
          " наблюдение %.4f" % (lin, b861))
    dev = 100 * (b861 - lin)
    print("  отклонение от линейности %+.1f п.п. -> КОЛЕНА НЕТ,"
          " B(E) монотонно падает" % dev)

    print("\nОстаток истинного завышения модели = B(E)*(eps_наша/eps_штат)-1:")
    for E in (860.557, 2614.511):
        r = (1 + EXCESS[E]) * b[E] - 1
        print("  %8.1f кэВ: (1+%.4f)*%.4f - 1 = %+.1f %%"
              % (E, EXCESS[E], b[E], 100 * r))
    r861 = (1 + EXCESS[860.557]) * b861 - 1
    r2614 = (1 + EXCESS[2614.511]) * b2614 - 1
    print("\nВЫВОД: после приведения к одной конвенции остаток на двух"
          " концах сходится (%+.1f %% и %+.1f %%)." % (100 * r861,
                                                       100 * r2614))
    print("  Разрыв на 2614,5 (+%.1f %% до поправки) — В ОСНОВНОМ артефакт"
          " конвенции съёма площади;" % (100 * EXCESS[2614.511]))
    print("  истинное завышение eps модели ~%.0f %% ПЛОСКО по диапазону."
          % (100 * 0.5 * (r861 + r2614)))
