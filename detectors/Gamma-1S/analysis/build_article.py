...
<div class="card warn"><p><b>Цель работы — проверка алгоритмов анализа
спектров, а не сама модель прибора.</b> Модель даёт эффективность
регистрации, связывающую площадь ППП с активностью. Мерой проверки служит
поверенный комплект: на каждую геометрию измерения имеется аттестованная
кривая эффективности, у каждого источника — паспортная активность с
погрешностью. Сорок поверочных спектров с паспортными активностями
позволяют установить, где цепочка обработки расходится и на сколько.</p></div>
