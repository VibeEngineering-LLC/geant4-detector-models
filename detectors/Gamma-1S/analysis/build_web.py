...
def answer_card(kit):
    """Главный результат для читателя: совпала ли восстановленная активность с
    паспортной и годится ли метод. Числа — из kit_activity(), не хардкод: при
    пересчёте комплекта карточка меняется вместе с таблицей сводки.

    Логика классификации: активность из спектра и расчётной эффективности
    делится на паспортную; отклонение (отношение − 1) в процентах. Точечные
    геометрии — репер самого детектора и алгоритмов (сосуда нет); сосуды —
    проверка модели сосуда поверх того же детектора.
    """
    def pct(key):
        v = kit.get(key)
        return None if not v else (v[0] - 1.0) * 100.0

    p5, p25 = pct("Point_5cm"), pct("Point_25cm")
    mar, den, pet = pct("Marinelli_1L"), pct("Denta_120mL"), pct("Petri_60mL")
    if None in (p5, p25, mar, den, pet):
        return ""  # неполные данные — молчим, а не печатаем полу-ответ
    pt_worst = max(abs(p5), abs(p25))

    def sgn(x):
        return ("завышает на %s %%" % ru(abs(x), 0)) if x > 0 \
            else ("занижает на %s %%" % ru(abs(x), 0))

    return (
        '<div class="card warn" id="answer" style="border-left-width:3px">'
        '<p><b>Главный результат: пригодна ли методика расчёта эффективности.</b> '
        'Проверка отвечает на один вопрос — совпадёт ли активность источника, '
        'восстановленная из его спектра по расчётной эффективности, с '
        'паспортной.</p>'
        '<p><b>На точечном источнике — да.</b> Обе точечные геометрии дают '
        'активность в пределах %s %% от паспортной (5 см — отношение %s, '
        '25 см — %s). Для точечной геометрии расчёт эффективности на известном '
        'ответе воспроизводит активность.</p>'
        '<p><b>На сосудах — расходится, и в разные стороны:</b> Маринелли '
        '%s (отношение %s), «Дента» %s (%s), Петри %s (%s). Знак расхождения '
        'меняется от сосуда к сосуду, поэтому его причина — не общий '
        'множитель детектора и не способ съёма площади (они проверены точечной '
        'геометрией), а модели самих сосудов, построенные по неполным '
        'чертежам. Разбор — в разделах «Сводка» и «Вывод».</p>'
        '<p>Практический вывод: методика расчёта эффективности по пику полного '
        'поглощения на поверенном комплекте подтверждена для точечной '
        'геометрии; перенос на объёмный сосуд требует его достоверной '
        'геометрии.</p>'
        '</div>'
        % (ru(pt_worst, 0),
           ru(kit["Point_5cm"][0]), ru(kit["Point_25cm"][0]),
           sgn(mar), ru(kit["Marinelli_1L"][0]),
           sgn(den), ru(kit["Denta_120mL"][0]),
           sgn(pet), ru(kit["Petri_60mL"][0])))
...
def ratio_chart(pairs, med, zones=None, mcfit=None, erange=None, mspan=None):
    ...
    if med == med:
        span.append(med)
    half = max(abs(v - 1.0) for v in span) * 1.10
    half = max(half, 0.06)
    lo, hi = 1.0 - half, 1.0 + half
    step = 0.05 if half <= 0.18 else (0.1 if half <= 0.45 else 0.2)
    ...
    for v, lab in ticks_e():
        ...
        s.append('<text class="ax" x="%.1f" y="%d" text-anchor="middle">%s</text>'
                 % (x, RH - 12, lab))
    kmax = int(math.floor(half / step + 1e-9))
    for i in range(-kmax, kmax + 1):
        v = 1.0 + i * step
        if not (lo <= v <= hi):
            continue
        cls = "one" if i == 0 else "grid"
...
<figcaption class="cap">Разрез расчётной модели в сборе (сосуд Маринелли 1 л
установлен на головку). Красным отмечены элементы, известные без числовых
...
показателем степенной зависимости отношения от энергии
<code>d&nbsp;ln&nbsp;R&nbsp;/&nbsp;d&nbsp;ln&nbsp;E</code>: если отношение
локально идёт как энергия в степени <i>n</i>, то это и есть <i>n</i> —
насколько круто отношение растёт или падает с энергией.</p>
...
плотностях пробы (0,60…1,00 г/см³ против аттестованных 1,60): смена
...
<figure>%s
%s
<p class="cap"><b>Рис.</b> Синяя линия — расчёт (Geant4, подгонка по узлам
сетки), оранжевые квадраты — измеренные точки аттестованной кривой, серые
полосы — области без эталонных линий, где сверка недоступна. Точки измерения —
%s, усы: паспортная погрешность; расчёт — %s, усы: статистика прогона. Наведите
курсор на точку, чтобы увидеть числа.</p>
</figure>
<figure>%s
<p class="cap"><b>Рис.</b> Отношение расчёт/измерение по линиям. Синие точки —
отношение по отдельной линии (усы — погрешность), зелёная штриховая — единица
(полное совпадение), сиреневые точки — то же отношение после деления на поправку
каскадного суммирования там, где она посчитана. Средневзвешенное (в логарифме,
как в отчёте) %s по %d точкам, разброс формы RMS %s %%; медиана %s. Расчётная
кривая проведена полиномом %d-й степени по log-log — степень выбрана скользящим
исключением узла, медианная ошибка предсказания %s %%.</p>
</figure>
<div class="tw"><table>
...
""" % (esc(title), esc(note), legend(True),
