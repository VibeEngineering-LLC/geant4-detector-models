...
GEOMS = [
    ("Маринелли 1 л", "Маринелли", "eff_rho1.60.csv",
     "Проба охватывает детектор с боков и сверху. ОИСН-16, ρ = 1,6 г/см³ — "
...
<p>Физика — <code>G4EmStandardPhysics_option4</code> и
<code>G4RadioactiveDecayPhysics</code> (Geant4 11.2.1). Кривые эффективности
...
прогонами полного распада; событие спектрометра отделено от события Geant4
...
<p>Сечения взаимодействия сверены с NIST XCOM через <code>G4EmCalculator</code>
на четырёх материалах в диапазоне 59,5…661,7 кэВ: среднее отклонение
−0,27 %% при принятом пороге 1 %%; смещение систематическое (пятнадцать из
шестнадцати значений отрицательны), но мало. Каждый выходной спектр несёт
штамп провенанса: отпечаток SHA1 исходников геометрии, запечённый в
исполняемый файл при сборке, полный набор аргументов прогона и измеренную
долю телесного угла розыгрыша; таблицы результатов объявляют наблюдаемую
машинно проверяемым заголовком.</p>
"""
...
площади, — сокращается тождественно. Ход отклика характеризуется наклоном
<code>d&nbsp;ln&nbsp;R&nbsp;/&nbsp;d&nbsp;ln&nbsp;E</code>.</p>

<div class="tw"><table>
<thead><tr><th>шаг последовательности</th><th class="n">наклон ЛСРМ</th>
<th class="n">наклон расчёта</th><th class="n">расхождение</th></tr></thead>
<tbody>
...
<p class="lead"><b>Цель работы — алгоритмы анализа спектров и их проверка,
а не модель прибора.</b> Модель даёт эффективность регистрации, связывающую
...
каждого источника — паспортная активность с погрешностью. Сорок записей с
заранее известными ответами позволяют установить, где цепочка обработки
расходится и на сколько.</p>
