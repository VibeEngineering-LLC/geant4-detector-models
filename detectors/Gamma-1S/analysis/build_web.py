...
<p>Сетку энергий незачем выбирать по вкусу: она обязана накрывать область
...
сводка молча взяла бы ближайшую посчитанную точку.</p>
...
def spectra_section():
    ...
    if skipped:
        blocks.append('<div class="card warn"><p><b>Записи, не попавшие в этот '
                      'раздел (%d).</b> Перечислены, потому что молчаливый '
                      'пропуск читается как «всё посчитано»:</p><ul>%s</ul>'
                      '</div>'
                      % (len(skipped),
                         "".join("<li>%s, %s — %s</li>"
                                 % (esc(g), esc(n), esc(why))
