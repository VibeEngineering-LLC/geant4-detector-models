...
# дотянуться до краёв. В перекрытии соседние ветви смешиваются линейно по
# log E — это и есть «сшивка».
#
# ЧИСЛО ЗОН И СТЕПЕНИ ПОДОБРАНЫ ПЕРЕБОРОМ (указание оператора), критерий —
# скользящее исключение узла (не χ² подгонки: его занижают лишние степени).
# Перебрано: 1 зона (ст. 3–6), все двухзонные разбиения по узлам сетки,
# трёхзонные в духе ЛСРМ. Победили ДВЕ зоны с широким перекрытием
# 338–662 кэВ, степени 5/2: RMS предсказания 0,72 % (5 см) и 1,7 % (25 см)
# против 0,85 % и 5,2 % у трёхзонной ЛСРМ-разметки. Смысл прозрачен: ниже
# ~660 кэВ кривая горбатая (фотоэффект + поглотители, нужна 5-я степень),
# выше — почти прямая в log-log (комптон, хватает 2-й); третья зона на
# 24 узлах только отнимает статистику. Разметка ЛСРМ оптимальна для ИХ
# сеток с иным числом и расстановкой точек, а не для любой кривой.
