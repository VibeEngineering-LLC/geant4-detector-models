...
ГРАНИЦА ПРИМЕНИМОСТИ, читать до того, как пользоваться числами. Здесь измеренная
площадь сравнивается с МОНОЭНЕРГЕТИЧЕСКОЙ эффективностью из сетки. Для линий,
стоящих в окне поодиночке, это верно. Для групп — нет: измеренная площадь несёт
несколько линий, а моноэнергетическая точка не несёт ни одной группы, и никакой
поправкой выхода это не закрывается. По торию разброс отношений остаётся почти
двукратным именно поэтому.

Метод, которым числа считаются НА САМОМ ДЕЛЕ, — kit_recalc.py: он уширяет
модельный спектр до разрешения прибора и снимает площадь тем же окном, так что
бленды учтены с двух сторон одинаково; там по тем же линиям тория разброс 11 %.
Этот скрипт остаётся грубой сверкой «сходится ли вообще» и годен для одиночных
линий: Cs-137, K-40, 2614,5 кэВ.
"""
import glob
import math
import os
...
ROI_FWHM = 1.25      # полуширина окна площади в долях ПШПВ (как в bm.peak_area)


def emit_yield(nuc, E, win):
    """Выход НА РАСПАД РОДИТЕЛЯ, просуммированный по ТОМУ ЖЕ окну, что площадь.

    ЗАЧЕМ ОКНО, А НЕ ОДНА ЛИНИЯ. Площадь из измеренного спектра снимается окном
    ±1,25 ПШПВ — на 911 кэВ это ±73 кэВ. В NaI туда попадают не только 911,204,
    но и 964,766 с 968,971 того же Ac-228. Если выход брать для ОДНОЙ линии, а
    площадь — для трёх, отношение считается по разным вещам: восстановленная
    эффективность завышается, и отношение МК/эксп по этой линии проваливается.
    Ровно так на 911 кэВ выходило 0,94 против 1,6–1,8 на соседних линиях.

    Критерий нераздельности взят не на глаз: предел Спарроу 0,85·ПШПВ при
    ПШПВ(E) = R₆₆₂·√(662·E)/100 — модель уширения для сцинтилляторов, тот же
    порог стоит в LineMerger у BecqMoni. Для 911 и 965 он даёт 49,5 кэВ против
    расстояния 53,6: подгонкой линии формально разделимы, но ОКНОМ ±73 кэВ не
    разделяются никак. Поэтому выход суммируется по окну, а не расщепляется.
    """
    ...
    for fn, (nuc, aspec, dpct, dt0, mass, _t12, lines) in KIT.items():
        ...
        for E in lines:
            ...
            if r is None:
                ...
                continue
            rate, drate, rbg, _ = r
            pg, src = emit_yield(nuc, E, ROI_FWHM * fw)
            if pg is None:
