...
ДОПУЩЕНИЕ О ПЛОТНОСТИ, названное явно. Аттестованные кривые Петри, Денты и
Маринелли снимались на пробах с плотностью из описи комплекта, а наши сетки
считались на 0,60 и 1,60 г/см³. Поэтому отклик по каждому объёмному шагу
приводится ВИЛКОЙ по этим двум плотностям, а не одним числом: вилка честно
показывает, сколько в ответе от самопоглощения, а сколько от геометрии.
Для Маринелли рабочей считается 1,60 (`export_curves.py`: «сверялась с .efr»).
...
# ЛСРМ и метки наших сеток (вилка по плотности там, где она есть).
SEQ = [
    ("точка 5 см", "Точечная-5см", ("p5cm",)),
    ("Петри 60 мл", "Петри", ("petri0.60", "petri1.60")),
    ("Дента 120 мл", "Дента", ("denta0.60", "denta1.60")),
