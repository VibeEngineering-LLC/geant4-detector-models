...
# Объявления наблюдаемых. Две таблицы — ДВЕ величины: построчная (по линиям) и
# сводная (по нуклидам, по правилу ЛСРМ). Одно объявление на обе означало бы,
# что штамп лжёт ровно там, где заводится против лжи.
OBS_LINES = {
    "quantity": "A_изм/A_пасп по КАЖДОЙ линии объёмной записи комплекта —"
                " отношение восстановленной удельной активности к паспортной"
                " на дату измерения",
    "area": "пик полного поглощения; подложка вычтена по полкам",
    "window": "окно и полка в долях ПШПВ — конвенция размытого спектра"
              " (common/py/becqmoni)",
    "shelf": "симметричные полки; вырез полос вылета НЕ применяется",
    "blurred": "измерение как есть; расчётная eps снята с размытой сетки",
    "selection": "usable=0 — линия хуже порога чистоты 0;95 и в активность"
                 " НЕ идёт; из таблицы не убирается: на ней проверяется"
                 " деконволюция",
}
OBS_ACT = {
    "quantity": "активность нуклида по правилу ЛСРМ — средневзвешенное по"
                " ГОДНЫМ линиям; величина СВОДНАЯ; не построчная",
    "area": "площади тех же линий; что в построчной таблице",
    "window": "окно и полка в долях ПШПВ — та же конвенция",
    "shelf": "симметричные полки; вырез полос вылета НЕ применяется",
    "blurred": "измерение как есть; расчётная eps снята с размытой сетки",
    "estimate": "столбец estimate называет; какая из двух оценок ЛСРМ взята —"
                "  взвешенная или по разбросу",
}


def _stamp(obs, inputs=None):
    return stamp.lines("detectors/Gamma-1S/analysis/kit_recalc.py", obs,
                       inputs=inputs,
