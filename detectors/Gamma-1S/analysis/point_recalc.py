...
BIN_KEV = 1.0  # шаг сетки депозит-спектра; каналы стоят в (i+0.5)*BIN_KEV


def nchan(lo, hi, bin_keV=BIN_KEV):
    """Сколько каналов сетки попадает в [lo, hi] — по СЕТКЕ, а не по ширине.

    ЗАЧЕМ ОТДЕЛЬНОЙ ФУНКЦИЕЙ. Каналы стоят в серединах, `(i+0.5)*bin`, то есть
    сетка смещена на полканала. Ширина окна в кэВ и число каналов в нём — не
    одно и то же, и «очевидная» формула `2*win+1` даёт 13 там, где каналов
    ровно 12: в ±6,0 кэВ вокруг ЛЮБОЙ энергии попадает 12 каналов, а не 13.
    Пересчитывать надо по сетке, тогда ошибка невозможна при любом шаге и
    любом смещении.
    """
    i0 = math.ceil(lo / bin_keV - 0.5)
    i1 = math.floor(hi / bin_keV - 0.5)
    return max(0, i1 - max(i0, 0) + 1)


def area_sim(hist, E, key=None, broad=True, win=6.0, bg0=25.0, bg1=10.0):
    ...
    ОКНО ПОЛКИ `[E−25, E−10]`, А НЕ `[E−30, E−10]` (method-rules §1). Прежнее
    окно захватывало пик вылета K-рентгена иода (`E−28,6`), занимающий 42…73 %
    его содержимого: полка вычиталась плоской подложкой, а внутри неё сидела
    дискретная особенность. Вся структура вылета лежит ниже `E−25` (иод Kα 28,6
    и Kβ 32,3; цезий Kα 31,0), поэтому окно годится и для CsI.

    ШИРИНА — ЧЕРЕЗ nchan(), А НЕ ФОРМУЛОЙ. Прежний множитель `2*win+1` считал
    13 каналов вместо 12 и завышал вычитаемую полку на 13/12 = 8,3 %. Это не
    описка: ошибка того же класса, что окно полки, — правило про сетку
    записывалось в кэВ, а применялось к каналам.
    """
    if broad:
    ...
    n_peak = nchan(E - win, E + win)
    n_side = nchan(E - bg0, E - bg1)
    if not n_side:
        return gross
