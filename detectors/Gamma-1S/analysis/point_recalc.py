    ...
    if summary:
        ...
        with open(sp, "w", encoding="utf-8", newline="") as fh:
            fh.write("# Активность по правилу ЛСРМ для точечных геометрий: "
                     ...
                     "# Строка нуклида: линии сводятся по СТАТИСТИКЕ, затем "
                     "в квадратуре добавлена\n"
                     "#   паспортная доля активности источника.\n"
                     "# nuclide=* — сводка по геометрии: по РЯДАМ НУКЛИДОВ, а "
                     "не по линиям, потому\n"
                     "#   что паспортная неопределённость независима у каждого "
                     "источника и поперёк\n"
                     "#   нуклидов она главная (статистика даёт 0,1–0,6 % на "
                     "линию). n_lines у этой\n"
                     "#   строки — сколько ГОДНЫХ ЛИНИЙ стоит за сводкой.\n"
                     "# chi2_dof — мера СОГЛАСИЯ рядов между собой. Больше ~2 "
                     "значит, что одним\n"
            ...
            fh.write("geometry,nuclide,n_lines,ratio,d_ratio,estimate,"
                     "chi2_dof\n")
            for r in summary:
                fh.write("%s,%s,%d,%.4f,%.4f,%s,%s\n"
                         % (r[0], r[1], r[2], r[3], r[4], r[5],
                            "" if r[6] is None else "%.2f" % r[6]))
