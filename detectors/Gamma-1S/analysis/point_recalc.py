...
# Из них строятся полосы исключения в полке размытого спектра.
ESCAPES = (28.6,)
...
OBS = {
    ...
    "shelf": "Point_5cm: симметричные по 1;0 ПШПВ с двух сторон;"
             " полосы вылета %s кэВ исключены"
             " | Point_25cm: %s"
             % ("/".join("%.1f" % e for e in ESCAPES), _DEP["shelf"]),
    "blurred": "Point_5cm: да (модель размыта до приборного ПШПВ)"
               " | Point_25cm: %s" % _DEP["blurred"],
    "escapes_keV": "/".join("%.2f" % e for e in ESCAPES),
