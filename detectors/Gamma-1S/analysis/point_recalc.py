...
#
# ВЫРЕЗ ПОЛОС ВЫКЛЮЧЕН (пустой кортеж), и это решение, а не недоделка.
#
# Замысел был исключать полосу вокруг `E − 28,6` из оценки полки. На МОДЕЛЬНОМ
# спектре шаг 1 кэВ и полка держит 15…50 каналов — вырез отнимает часть. На
# ИЗМЕРЕННОМ 1024 канала на ~3 МэВ, то есть ~3 кэВ на канал, и полка шириной
# 1 ПШПВ держит ВСЕГО 7…8 каналов: вырез ±0,5·ПШПВ съедает 6…7 из них, порог
# `keep_frac` не выполняется, и съём молча уходит на ОДНОСТОРОННЮЮ полку.
# Измерено на записях комплекта (задача 135): Ba-133 81,0 — остался 1 канал из
# 7, Cd-109 88,0 — 0 из 7, Co-57 122,1 — 1 из 8. Скорость счёта сдвигалась на
# −6,9 %, +12,9 %, +11,8 %, +15,2 % — это не работа выреза, а откат, заново
# вносящий чувствительность к наклону континуума, ради устранения которой
# полка и делалась двусторонней.
#
# Отказ от выреза ОБОСНОВАН НЕ ТЕМ, что заражение сокращается в отношении:
# такое сокращение держалось бы на допущении, что модель передаёт вылет верно,
# а проверить это на записях комплекта не удалось — окно вылета накрывает
# собственные линии источников (у Ba-133 линия 53,162 против ожидаемого вылета
# 52,4), см. `escape_balance.py`. Отказ обоснован тем, что вырез на измеренной
# стороне ДОКАЗАННО ломает полку, а неисправная поправка хуже объявленной
# систематики.
#
# Поэтому: обе стороны считаются ОДНИМ правилом без выреза, а заражение полки
# объявляется в наблюдаемой как названная неустранённая систематика с указанием
# полосы действия (54,3…217,4 кэВ для иода) — см. ключ `obs.escape_bias`.
ESCAPES = ()

# Полоса энергий, где вылет попадает в полку конвенции ±1 ПШПВ: выводится из
# `ПШПВ(E) ≤ E_выл ≤ 2·ПШПВ(E)` при `ПШПВ = k·√E`. Печатается в объявлении,
# чтобы потребитель знал, на каких линиях систематика действует.
ESCAPE_KEV = 28.6
FWHM662 = 49.9        # измерено по пику 662 записи цезия (7,5 %), паспорт ≤8 %
_K = FWHM662 / math.sqrt(661.657)
ESCAPE_BAND = ((ESCAPE_KEV / (2 * _K)) ** 2, (ESCAPE_KEV / _K) ** 2)
...
OBS = {
    ...
    "escape_bias": "НЕ УСТРАНЕНА: пик вылета K-рентгена иода (%.1f кэВ) попадает"
                   ...
                   " (линии 59;5; 81;0; 88;0; 122;1; 165;9)."
