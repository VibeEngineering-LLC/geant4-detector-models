...
#
# ЕДИНАЯ КОНВЕНЦИЯ НА ОБЕИХ ДИСТАНЦИЯХ (задача 132, решение оператора
# 31.07.2026: свести определения). До этой правки 5 см и 25 см считались
# по РАЗНЫМ определениям площади — 5 см через eps_decay_5cm ->
# area_sim(broad=True) (спектр размыт приборным ПШПВ, окно ±1,0 ПШПВ с
# симметричными полками), 25 см через eps_mono_point -> area_sim(broad=False)
# (нЕразмытый депозит, депозитное окно peakwin). Расхождение 5 против 25 см
# было отчасти конвенцией съёма площади, а не геометрией физики — прямое
# сравнение дистанций было некорректно. Обе стороны теперь идут через
# area_sim(broad=True): eps_mono_point тоже размывает сетку моноэнергий и
# снимает площадь тем же окном ±1,0 ПШПВ, что и измерение (bm.net_rate).
OBS = {
    ...
    "window": "+-1;0 ПШПВ (единая конвенция на обеих дистанциях; задача 132)",
    "shelf": "симметричные по 1;0 ПШПВ с двух сторон; вырез полос вылета"
             " НЕ применяется (см. obs.escape_bias)",
    "blurred": "да — обе стороны (5 и 25 см) размыты до приборного ПШПВ;"
               " включая сетку моноэнергий на 25 см",
...
def eps_mono_point(gtag, E):
    ...
    # РАЗМЫТИЕ + ПШПВ-окно — тем же правилом, что и 5 см (задача 132,
    # решение оператора 31.07.2026: свести конвенции). Прежде здесь стояло
    # broad=False (узкое депозитное окно в каналах): обоснование было верным
    # само по себе — в сетке моноэнергий блендов нет, узкое окно ничего не
    # теряет, — но конвенция ПЕРЕСТАВАЛА совпадать с измеренной стороной,
    # которая всегда идёт через bm.net_rate в долях ПШПВ (реальный спектр
    # размыт инструментом по определению). Разные окна на двух сторонах ОДНОЙ
    # дроби делали 5 и 25 см несравнимыми не по физике, а по счёту.
    a = area_sim(hist, E, key="mono:%s:%.1f" % (gtag, E), broad=True)
        ...
        for nuc, (tag, lines) in NUC.items():
            ...
            for E in lines:
                ...
                if geom == "Point_5cm":
                    ...
                    # ФИЛЬТР ОСТАЁТСЯ ТОЛЬКО НА 25 СМ И ПОСЛЕ ЗАДАЧИ 132 —
                    # не пропущенный by-product старой асимметрии. Причина не
                    # в окне съёма (теперь на обеих дистанциях одно и то же),
                    # а в ИСТОЧНИКЕ модельного спектра: на 5 см модель — это
                    # прогон ПОЛНОГО распада (p5_<нукл>.csv), в нём те же
                    # каскадные линии, что видит измерение, поэтому загрязнение
                    # полки СОКРАЩАЕТСЯ в отношении модель/измерение. На 25 см
                    # прогонов распада нет вовсе (см. докстринг файла) —
                    # модель это СЕТКА МОНОЭНЕРГИЙ, одна линия без соседей по
                    # построению; размытие её не добавляет соседей, которых в
                    # данных нет. Загрязнение измеренной полки поэтому НЕ
                    # сокращается на 25 см, и фильтр обязателен только там.
                    # (найдено вторым независимым аудитом задачи 132)
                    ...
                    # em5 — служебная величина ТОЛЬКО для оценки поправки на
                    # суммирование C5/C25 (сравнение сетки моноэнергий с
                    # прогоном распада на ОДНОЙ дистанции, 5 см). С задачи 132
                    # eps_mono_point тоже размыт и снят ПШПВ-окном — то есть
                    # em5 и ed5 теперь сравниваются ОДНИМ окном (были разными
                    # до правки), и C5/C25 сместились как побочный эффект
                    # унификации, а не только em/ed на 25 см. Не отмечено в
                    # первой редакции правки — записано вторым аудитом.
