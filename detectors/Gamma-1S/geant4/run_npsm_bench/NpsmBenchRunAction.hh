...
public:
    ...
    // нуклида в разных кюветах или в разной матрице иначе неразличимы по
    // шапке — тот же класс, что W-068, только про пробу, а не про физику.
    // Умолчания воспроизводят прежнее поведение: кюветы нет, источник точечный.
    static std::string gVessel;    // "none" | marinelli | denta | petri | …
    static std::string gMatrix;    // имя матрицы пробы (risn379, OISN16, …)
    static double gRhoSample;      // г/см³, 0 — кюветы нет
    static double gSampleCm3;      // объём засыпки, 0 — кюветы нет
    static std::string gSrcMode;   // "point" | "sample"
    // Ограничение цепочки распада (nucleusLimits), пусто — без ограничения.
    static std::string gChainLimits;

    // Экран-защита: 1 — построена, 0 — снята (ключ shield). Обязана лежать
    // В ФАЙЛЕ по той же причине, что gVessel и gThreads. Найдено 12.09.2026
    // при разборе вклада защиты: шапки пары shld1/shld0 не различались НИЧЕМ,
    // и принадлежность файла к варианту постановки несло только его имя —
    // рецидив W-068. Умолчание 1 воспроизводит прежнее поведение.
    static int gShield;
    // Толщины вкладышей экрана Cu и Cd, мм — применённые, не запрошенные
    // (14.09.2026, прогон 26в). -1 — стенд без экрана Гамма-1С, в шапку не пишутся.
    static double gLinerCuMm;
    static double gLinerCdMm;
    // Вариант геометрии защиты (15.09.2026, #GEO-1): пишется в шапку ВСЕГДА,
    // умолчание "legacy" — без строки шаблоны прежней защиты и geo1 различались
    // только именем файла (рецидив W-068).
    static std::string gShieldVariant;
