...
def main():
    ...
    # ⚠ РАЗГАДКА W-071 (11.09.2026). glob возвращает файлы в порядке файловой
    # системы — по ИМЕНАМ, не по энергии: «1000, 100, 1500, 2000, 200, …».
    # Критерии С-1 и С-2 проверяют МОНОТОННОСТЬ ряда и без этой сортировки
    # проверяли её в порядке имён файлов. Отсюда невоспроизводимость приёмки
    # этапа 3: один и тот же анализатор на одних и тех же данных давал разный
    # вердикт, стоило поменять имена набора (s3_* против s3mt_*) — менялся
    # алфавитный порядок. Значения σ при этом всегда были верны; неверен был
    # порядок, в котором их сравнивали.
    results.sort(key=lambda r: r["E_keV"])
