...
> F-331 / v1.18.18.5 — Canonical "sample + background" kits
> for use in regression and acceptance tests.
...
├── {geometry}/
│   └── {nuclide}/
│       ├── sample_{...}.spe        — reference source spectrum
│       └── background_{...}.spe    — relevant averaged background
```

All other historical spectra from the legacy folder
`Gamma-1C_NaI_63x63_USB_SN-01/` have been moved to
`detectors/Gamma-1C/reference_spectra/archive/`, preserving
...
| Point_25cm | Na-22 | `sample_Na-22 #SRC-12_Точечная-25см_25cm.spe` (0fa53e2ecb) | `background_bg_2016_open_lid_point25cm.spe` (5d6a9751da) |
| Point_25cm | Th-228 | `sample_Th-228 №SRC-03_Точечная-25см_25cm.spe` (7a00a12d61) | `background_bg_2016_open_lid_point25cm.spe` (5d6a9751da) |
...
Passport activities for Marinelli are automatically read from
the `COMMENT` field of the .spe file (F-330 v1.18.18.4) — there is no
need to pass `passport_activity_Bq`.
