...
def render(cfg):
    ...
    for r in runs:
        ...
        # z_min/z_max (опционально, по умолчанию оба = r["z"]) -- ДОБАВЛЕНО
        # 11.08.2026 (источник AmTiCsEu, задача геометрия Ti-44): для
        # нуклида, чья ЕДИНСТВЕННАЯ наблюдаемая сильная линия испускается
        # НЕ им самим, а прямым потомком с коротким T1/2 внутри ТОГО ЖЕ
        # массового числа (Ti-44 EC-> Sc-44, T1/2=4,042 ч -- секундная
        # шкала по сравнению с годами эксплуатации источника, полное
        # вековое равновесие), одного Z недостаточно: nucleusLimits с
        # единственным Z замораживает дальнейший распад дочернего ядра,
        # и его собственная гамма-линия (для Sc-44 -- 1157,02 кэВ) не
        # появится в шаблоне вовсе. z_min/z_max задают ДИАПАЗОН Z, в
        # котором распад разрешён РАДИОАКТИВНОМУ МОДУЛЮ (RDM) -- ПРОМПТ-
        # гамма де-возбуждение (PhotonEvaporation) этим диапазоном не
        # управляется никогда, оно всегда включено для непосредственного
        # продукта распада. НЕ применять этот приём к нуклиду вроде Am-241,
        # чей физический потомок (Np-237, T1/2=2,14e6 лет) в реальном
        # источнике распасться не успевает -- там Z должен остаться
        # единственным, иначе принудительный порог thresholdForVeryLong-
        # DecayTime заставит Np-237 молча распасться в модели.
        z_min = r.get("z_min", r["z"])
        z_max = r.get("z_max", r["z"])
        lines.append("")
        lines.append("# --- %s (Z=%d, A=%d%s%s) ---"
                      % (r["key"], r["z"], r["a"],
                         ", E*=%g кэВ" % exc if exc else "",
                         ", Z_range=%d-%d" % (z_min, z_max)
                         if (z_min, z_max) != (r["z"], r["z"]) else ""))
