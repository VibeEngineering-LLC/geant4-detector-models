...
def _sum_peaks_with_fb(pairs, path=None):
    ...
    которую прежний экспорт не забирал). Список нуклидов там расширен
    09.08.2026 вечером (аудит Б4, #2) на ОБЕ цепочки целиком — изначально
    (Б1) был узкий, только Ac228/Tl208/Bi214, но tools/enumerate_sum_
    peaks.py тоже использует этот файл и должен уметь считать F_B для
    ЛЮБОГО нуклида библиотеки, не только уже отобранных пар; для перехода
    без найденного CC подставляется 0 (чистый радиационный канал,
    поведение как в старом коде) — молча, но со счётчиком в конце
...
def assert_anchor_window_ok(window_half_kev, E_kev, max_mult=0.7, label=""):
    ...
    Зачем (ТЗ внешнего аудита, ретроспектива Теста 3 AmTiCsEu,
...
def _multiplet_chi2(theta, x, y, lines, E0, w0, p_law):
    ...
    ПРОБОВАЛОСЬ 11.08.2026 (директива «правь калибровку, примени
    деконволюцию», разбор истории Ra-226 ra226-remarks.md §16): подложка
    ступенью erfc вместо/вместе с наклоном (модель Debertin/Helmer). На
    якорях Th-232 (слитно с наклоном) не хуже нигде, лучше на 3 из 6 --
    но задачу калибровки НЕ решила: на AmTiCsEu чистые линии 661,657 и
    1408,013 (chi2/ndof 1,9 и 1,25, статистически хороший фит) дали
    сдвиг -3,2 и -10,9 кэВ при независимо подтверждённой шкале ≤1,1 кэВ
    -- метод врёт уверенно, не только на плохих фитах. Оператор указал
    (скриншот оригинала SpectraLine): у ЭТАЛОННОГО ПО, чьи данные вся
    линия проверяет с самого начала, подложка тоже БЕЗ ступени, гладкая
    наклонная/квадратичная. Ступень отброшена целиком, отклонение от
    эталонной методологии без выигрыша не оправдано.
    """
