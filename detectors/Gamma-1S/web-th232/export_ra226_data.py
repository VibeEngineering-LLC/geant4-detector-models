...
def main():
    ...
    # ── энергетическая шкала: ЗАВОДСКАЯ, без поправок.
    #
    # ИСТОРИЯ (важна, чтобы не наступить второй раз). 10.08.2026 сюда была
    # добавлена линейная поправка E = a + b·E_заводское, снятая по
    ...
    # «уехал влево почти на 200 кэВ».
    #
    # Причина ошибки: центроиды из ed.fit_peak_multiplet систематически
    # занижены (все пять -- в одну сторону, от -3,4 до -22,4 кэВ). Модель
    # окна там -- гауссианы плюс ПРЯМАЯ подложка, а под пиком реально
    # стоит комптоновская СТУПЕНЬ (слева от пика континуум выше, чем
    # справа). Прямая её не описывает, избыток слева фиттер компенсирует
    # сдвигом гауссианы влево. Это артефакт подгонки, а не смещение
    # шкалы -- но был принят за смещение шкалы и «исправлен».
    #
    # Заводская шкала проверена тремя независимыми способами, все три
    # подтверждают, что она верна и поправки не требует:
    #  1. СпектраЛайн (стороннее ПО, скриншот оператора): пики на каналах
    #     594,2 / 740,9 / 821,0 -> 1763,764 / 2203,936 / 2444,631 кэВ.
    #     Заводской полином на тех же каналах даёт 1763,5 / 2203,8 /
    #     2444,5 -- расхождение 0,12-0,25 кэВ.
    #  2. SpectraVibe (gamma.cli analyze --full-report, свой конвейер
    #     поиска/идентификации/калибровки): seven_line_check по ЕРН-линиям
    #     -- max|Δ| = 8,3 кэВ = 9% ПШПВ при пороге 30%, вердикт "ok".
    #  3. Сам полином (4-я степень, шапка .spe): гладкий, строго
    #     монотонный, шаг 2,80-3,14 кэВ/канал, без выбросов.
    #
    # Что делать, если когда-нибудь понадобится РЕАЛЬНАЯ перекалибровка:
    # брать центроиды не отсюда, а из gamma.peaks.centroid_gost (ГОСТ
    # 26874-86 § 3.3.3, с вычитанием пьедестала по § 3.3.2) либо из
    # gamma.calibration.anchor_recalibration -- там подложка под пиком
    # обрабатывается корректно. И сверять результат со сторонним ПО ДО
