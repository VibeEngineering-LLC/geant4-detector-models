...
def main():
    ...
    # ── Ra-226 ЗАКРЕПЛЁН на паспортной активности, утечка -- по остатку
    # (10.08.2026 вечером, оператор: «подгоняй радий под сумму и смотрим
    # что с утечкой»). Смысл: 186,211 кэВ (родитель) профессионально не
    # берут для активности (самопоглощение, слабая линия) -- вместо того
    # чтобы измерять родителя ЧЕРЕЗ эту линию (как в 2-/3-амплитудном
    # разрезе выше), здесь родитель ФИКСИРУЕТСЯ на A_pass (паспорт,
    # decay-corrected), и свободной остаётся ТОЛЬКО амплитуда дочерних --
    # оценка утечки не зависит от того, насколько хорошо/плохо модель
    # держит именно эту одну проблемную линию.
    model_parent_fixed = shape_parent[sel] * T * A_pass
    y_residual = y_sel - model_parent_fixed
    cols_fx = [shape_daughter[sel] * T, bgm]
    coef_fx, dcoef_fx, chi2_fx, ndof_fx, model_fx = ed.fit_amplitudes(
        y_residual, cols_fx, ed.SYS_FLOOR)
    A_dtr_fx, dA_dtr_fx = float(coef_fx[0]), float(dcoef_fx[0])
    ratio_fx = A_dtr_fx / A_pass
    d_ratio_fx = ratio_fx * (dA_dtr_fx / A_dtr_fx) if A_dtr_fx > 0 else float("nan")
    print("утечка (Ra-226 закреплён на паспорте=%.0f Бк): дочерние=%.0f+-%.0f"
          " Бк  отношение=%.3f+-%.3f  chi2/ndof=%.2f -- утечка~%.0f%%, "
          "СПРАВОЧНО"
          % (A_pass, A_dtr_fx, dA_dtr_fx, ratio_fx, d_ratio_fx,
             chi2_fx / ndof_fx, 100.0 * (1.0 - ratio_fx)))
    radon_fixed_parent_check = {
        "attempted": True, "method": "parent_fixed_at_passport",
        "A_parent_fixed_Bq": A_pass,
        "A_daughter_Bq": A_dtr_fx, "dA_daughter_Bq": dA_dtr_fx,
        "ratio_daughter_to_parent": ratio_fx, "d_ratio": d_ratio_fx,
        "leak_fraction_pct": 100.0 * (1.0 - ratio_fx),
        "chi2_ndof": chi2_fx / ndof_fx,
        "reliable": False,
        "caveat": "Родитель Ra-226 ЗАКРЕПЛЁН на паспортной активности "
                  "(decay-corrected, %.0f Бк), НЕ измеряется через линию "
                  "186,211 кэВ (та же причина, по которой её "
                  "профессионально не используют для активности -- "
                  "самопоглощение/слабая линия). Свободна ТОЛЬКО амплитуда "
                  "дочерних (Rn222+Pb214+Bi214+Po218/214). Результат: "
                  "отношение %.3f -- дочерние читаются на ~%.0f%% ВЫШЕ "
                  "(не ниже) паспортного родителя, т.е. РЕАЛЬНОЙ утечки "
                  "этот тест НЕ ПОКАЗЫВАЕТ (отрицательная 'утечка' в "
                  "пределах шума фита, chi2/ndof=%.2f). РЕШАЮЩИЙ ВЫВОД "
                  "(10.08.2026 вечером): избыток, найденный в 2-/3-"
                  "амплитудных разрезах выше (родитель 1,27-1,30x "
                  "паспорта), СИДИТ ЦЕЛИКОМ в измерении родителя через "
                  "линию 186,211 кэВ -- как только эта линия ИСКЛЮЧЕНА из "
                  "измерения (родитель зафиксирован извне, паспортом), "
                  "дочерние сходятся с ожиданием. Гипотеза (а) реальной "
                  "утечки Rn-222 -- ОСЛАБЛЕНА этим тестом (дочерние не "
                  "занижены, направление обратное). Гипотеза (б) -- "
                  "систематика модели/самопоглощения именно на 186,211 "
                  "кэВ в новой матрице OISN06_epoxy (первый прогон) -- "
                  "УСИЛЕНА: она одна объясняет и завышение родителя, и "
                  "нормальное поведение дочерних. Ни (а), ни (б) не "
                  "проверены до конца независимым способом (для (б) нужна "
                  "проверка чувствительности эффективности 186 кэВ к "
                  "составу/плотности матрицы) -- reliable=false, но (б) "
                  "теперь предпочтительнее (а) по совокупности."
                  % (A_pass, ratio_fx, 100.0 * (1.0 - ratio_fx),
                     chi2_fx / ndof_fx),
    }
    ...
    data = {
        ...
        "radon_fixed_parent_check": radon_fixed_parent_check,
    }
