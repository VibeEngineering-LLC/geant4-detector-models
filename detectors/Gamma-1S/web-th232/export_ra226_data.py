...
def main():
    ...
    # Обычный (невзвешенный) МНК по 5 якорям. Взвешивание по 1/d_shift^2
    # (d_shift -- статистическая погрешность сдвига из кривизны χ² той же
    # деконволюции, ed.fit_peak_multiplet) ПРОВЕРЕНО и ОТКЛОНЕНО 10.08.2026:
    # статистическая погрешность (0,04-1,1 кэВ) на порядок меньше реального
    # разброса между якорями (до 7 кэВ) -- разброс систематический (модель
    # подложки в окне деконволюции линейна, на широких окнах высоких энергий
    # континуум Комптона так не выглядит), не статистический. Взвешивание по
    # заниженной статistической погрешности утянуло фит почти целиком к
    # самому "уверенному" (351,932 кэВ) якорю и увеличило реальную невязку
    # на 2204,100 кэВ до 14,9 кэВ (было 5,9 кэВ) -- хуже, не лучше. Квадратичная
    # поправка (3 параметра на 5 точек) тоже проверена: снижает СКО невязки
    # с 4,3 до 2,4 кэВ, но остаточный знакопеременный паттерн без видимого
    # тренда типичен для подгонки шума, не искривления шкалы -- отклонена
    # (см. design note в SESSION-STATE про риск переобучения на 5 точках).
    # Оставлена простая линейная невзвешенная поправка; d_shift_keV/resid_keV
    # сохранены в meta как диагностика для будущей работы.
