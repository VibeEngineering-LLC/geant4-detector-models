...
def main():
    ...
    radon_check = {
        ...
        "caveat": "Метод -- деконволюция ПО ВСЕМУ спектру двумя независимыми "
                  ...
                  "зависимость величины от окна (1,05...1,33) остаётся "
                  "оговоркой модели формы -- поэтому количественная оценка "
                  "утечки берётся из теста ДПР-против-Ra-226 (две свободные "
                  "амплитуды, рабочее окно), а не из этого разреза.",
    }
    ...
    # Коэффициент эманирования f = 1 - ДПР/Ra-226 (принятая версия, решение
    # оператора 10.08.2026: источник аттестован ПО ДПР, при постоянной
    # утечке аттестация видела ту же подавленную ДПР-активность, что и
    # это измерение -- поэтому ДПР~паспорт, а линия 186,211 кэВ меряет
    # сам Ra-226 напрямую, ДО утечки). Погрешность -- по квадратурам,
    # ковариация совместного фита двух амплитуд не учтена (оценка
    # приближённая).
    ...
    print("ДПР против Ra-226 (обе свободно, по форме): ДПР=%.0f+-%.0f Бк "
          ...
          "к-т эманирования f=%.1f%%+-%.1f%%  chi2/ndof=%.2f -- ПРИНЯТАЯ ВЕРСИЯ"
          % (A_dpr, dA_dpr, A_dpr / A_pass, A_ra, dA_ra, A_ra / A_pass,
             leak_frac * 100, d_leak_frac * 100, chi2_2 / ndof_2))
    radon_dpr_vs_ra_check = {
        ...
        "accepted": True,
        "caveat": "ПРИНЯТАЯ ВЕРСИЯ (решение оператора 10.08.2026): утечка "
                  "радона. Обе амплитуды СВОБОДНЫ (по форме спектра, метод "
                  "2), ничего не зафиксировано на паспорте: ДПР (Pb-214+"
                  "Bi-214 -- линии, которыми активность Ra-226 определяется "
                  "по стандартной практике) = %.2f паспорта, Ra-226 (линия "
                  "186,211 кэВ) = %.2f паспорта. Источник аттестован ПО "
                  "ДПР -- при ПОСТОЯННОЙ утечке радона аттестация видела "
                  "ту же подавленную ДПР-активность, что и это измерение, "
                  "поэтому согласие ДПР~паспорт и избыток Ra-226 над "
                  "паспортом -- ровно та картина, которую утечка и "
                  "предсказывает. Истинная активность Ra-226 в источнике "
                  "= A_Ra226_Bq (по линии 186,211 кэВ, до утечки); "
                  "коэффициент эманирования f = 1 - ДПР/Ra-226 = "
                  "%.1f%% +- %.1f%% -- по порядку согласуется с "
                  "литературными значениями для насыпных/гранулированных "
                  "матриц (~25%% для гранулированного стекла, Povinec "
                  "2015, §19 remarks). "
                  "Оговорки: ковариация совместного фита не учтена "
                  "(погрешность f приближённая); линия 186,211 кэВ слабая "
                  "(I=3,565%%), её самопоглощение в новой матрице "
                  "независимо не перепроверялось. Контроль на будущее: "
                  "при переизмерении источника ДПР/паспорт должно "
                  "оставаться ~1, Ra-226/ДПР ~ 1/(1-f)."
                  % (A_dpr / A_pass, A_ra / A_pass,
                     leak_frac * 100, d_leak_frac * 100),
    }
