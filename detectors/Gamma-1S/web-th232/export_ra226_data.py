...
def main():
    ...
    # Метаданные ПРИМЕНЁННОЙ поправки (раньше здесь стоял хардкод None --
    # найдено аудитом кода 11.08.2026: поправка реально применяется двумя
    # строками выше, но JSON заявлял её отсутствие). Числа -- буквально те
    # же константы, что использует apply_xray_anchor_correction_sample/_bg.
    energy_correction = {
        "applied": True,
        "method": "xray_anchor_local_quadratic_blend_to_factory",
        "anchor_kev": float(_XRAY_ANCHOR_KEV),
        "sample_anchor_channels": _SAMPLE_ANCHORS_CH.tolist(),
        "sample_anchor_kev": _SAMPLE_ANCHORS_KEV.tolist(),
        "blend_lo_ch": float(_BLEND_LO), "blend_hi_ch": float(_BLEND_HI),
        "note": "Заводская шкала верна выше канала %d (blend_hi); ниже "
                "заменена локальным квадратиком по 6 якорям, сшитым "
                "smoothstep-переходом. Подробности -- ra226-remarks.md §16."
                % int(_BLEND_HI),
    }
    ...
    # подгонки (E_FIT_LO=100 фикс.; испытанные E_FIT_HI 400/700/1200/2300
    # кэВ дали отношение дочерние/родитель 0,92/0,77/0,77/0,81 -- см.
    # caveat ниже) -- не пересчитывается на каждый прогон (не тот случай,
    # где нужна автоматизация), проверено вручную 10.08.2026, см. журнал
    # сессии; отдельно проверена чувствительность к E_FIT_LO (11.08.2026,
    # аудит кода + самостоятельная проверка) -- см. caveat
    # radon_dpr_vs_ra_check ниже.
    ...
    radon_check = {
        ...
        "caveat": "Метод -- деконволюция ПО ВСЕМУ спектру двумя независимыми "
                  ...
                  "Направление (родитель выше дочерних при ЛЮБОМ окне) "
                  "согласуется с принятой версией утечки радона (решение "
                  "оператора 10.08.2026, см. radon_dpr_vs_ra_check ниже). "
                  "ВАЖНО: у Rn222/Po218/Po214 нет ни одной линии в "
                  "библиотеке (их базисы -- нулевые векторы), поэтому "
                  "'родитель' здесь и 'Ra226' в radon_dpr_vs_ra_check -- "
                  "ЭТО ОДИН И ТОТ ЖЕ ФИТ, только другая группировка меток "
                  "(A_parent_Bq численно равен A_Ra226_Bq, A_daughter_Bq "
                  "равен A_DPR_Bq -- проверено побитово аудитом кода "
                  "11.08.2026). Оценка утечки в radon_dpr_vs_ra_check не "
                  "НЕЗАВИСИМА от этого разреза, а его переименование; "
                  "зависимость величины от окна (1,05...1,33 здесь = "
                  "диапазон f 7,7-23,0% там) -- ОДНА И ТА ЖЕ систематика "
                  "модели формы, перенесённая в оба места.",
    }
    print("разрез родитель/дочерние: A(родитель)=%.0f+-%.0f Бк  "
          "A(дочерние)=%.0f+-%.0f Бк  отношение=%.3f+-%.3f  chi2/ndof=%.2f  "
          "cond=%.2e  A_par/паспорт=%.2f -- СПРАВОЧНО (количественная "
          "оценка -- в тесте ДПР/Ra-226 ниже)"
    ...
    # определяется ПО ДПР (Pb-214/Bi-214), не по собственной линии
    # 186,211 кэВ -- значит и ПАСПОРТНАЯ активность источника получена
    # ПО ДПР. Поэтому предыдущая версия теста (фиксация "родителя" на
    # паспорте) была КОНЦЕПТУАЛЬНО НЕВЕРНОЙ -- паспорт сопоставляется с
    # ДПР-стороной, а Ra-226 мерится НЕЗАВИСИМО и сравнивается с обоими.
    # ФИНАЛ (замечание оператора, 10.08.2026 поздно вечером): ДПР
    # сходится с паспортом, Ra-226 выше -- при постоянной утечке радона
    # это НЕ противоречие, а предсказание: аттестация (по ДПР) видела ту
    # же подавленную активность. Версия утечки ПРИНЯТА, активность
    # пересчитана: истинный Ra-226 -- по линии 186,211 кэВ, доля утечки
    # f = 1 - ДПР/Ra-226.
    #
    ...
    # Доля активности Ra-226, не проявляющаяся в счёте ДПР этого сосуда:
    # f = 1 - ДПР/Ra-226 (принятая версия, решение оператора 10.08.2026 --
    # источник аттестован ПО ДПР, при ПОСТОЯННОЙ утечке аттестация видела
    # ту же подавленную ДПР-активность, что и это измерение -- поэтому
    # ДПР~паспорт, а линия 186,211 кэВ меряет сам Ra-226 напрямую).
    # "Источник аттестован по ДПР" -- ДОПУЩЕНИЕ, не подтверждённый факт:
    # свидетельства аттестации в конфиге/паспорте нет, methodika могла
    # быть и альфа-счётом (РД 50-373-82), при котором паспорт утечкой НЕ
    # подавлен и весь вывод не работает (см. §19 remarks). "Постоянство"
    # утечки 2002->измерение -- тоже допущение, не проверено.
    #
    # Статистическая погрешность f -- по квадратурам, БЕЗ ковариации
    # совместного фита (собственная проверка 11.08.2026: точная ковариация
    # даёт эффект ~2%, несущественна) и БЕЗ масштабирования на chi2/ndof=
    # 3,86 (масштабирование увеличило бы статистическую часть в ~1,96
    # раза). Но обе поправки МЕЛКИЕ рядом с СИСТЕМАТИКОЙ окна подгонки --
    # прямая проверка (11.08.2026, аудит кода + самостоятельный прогон,
    # НЕ принято на слово): при E_FIT_HI 400/700/1200/2300 кэВ (E_FIT_LO=
    # 100 фикс.) f = 7,7/23,0/22,8/19,5%; при E_FIT_LO 100/170 кэВ
    # (E_FIT_HI=2300 фикс.) f = 19,5/12,5%. Разброс по окну -- 7,7...23,0%,
    # НА ПОРЯДОК шире заявляемой статистической ±3,7% при рабочем окне.
    # Публикуется РАБОЧАЯ точка (100-2300 кэВ, та же граница, что у метода
    # 2 целиком) с честным диапазоном systematики, а не одно число с узкой
    # статистической оценкой.
    if A_ra > 0 and A_dpr > 0:
        leak_frac = 1.0 - A_dpr / A_ra
        d_leak_frac = (A_dpr / A_ra) * float(
            np.sqrt((dA_dpr / A_dpr) ** 2 + (dA_ra / A_ra) ** 2))
    else:
        leak_frac, d_leak_frac = float("nan"), float("nan")
    LEAK_FRAC_WINDOW_LO, LEAK_FRAC_WINDOW_HI = 0.077, 0.230  # см. комментарий выше
    print("ДПР против Ra-226 (обе свободно, по форме): ДПР=%.0f+-%.0f Бк "
          "(%.2fx паспорта)  Ra-226=%.0f+-%.0f Бк (%.2fx паспорта)  доля "
          "недостающая в ДПР f=%.1f%% (окно %.1f-%.1f%%)  chi2/ndof=%.2f "
          "-- ПРИНЯТАЯ ВЕРСИЯ, ДОПУЩЕНИЕ"
          % (A_dpr, dA_dpr, A_dpr / A_pass, A_ra, dA_ra, A_ra / A_pass,
             leak_frac * 100, LEAK_FRAC_WINDOW_LO * 100,
             LEAK_FRAC_WINDOW_HI * 100, chi2_2 / ndof_2))
    radon_dpr_vs_ra_check = {
        ...
        "leak_fraction": leak_frac, "d_leak_fraction": d_leak_frac,
        "leak_fraction_window_range": [LEAK_FRAC_WINDOW_LO,
                                       LEAK_FRAC_WINDOW_HI],
        "chi2_ndof": chi2_2 / ndof_2,
        "accepted": True,
        "caveat": "ПРИНЯТАЯ ВЕРСИЯ (решение оператора 10.08.2026), но "
                  "СТРОИТСЯ НА ДОПУЩЕНИИ, не на подтверждённом факте: "
                  "источник считается аттестованным ПО ДПР, а утечка -- "
                  "постоянной с 2002 года. Подтверждающего документа в "
                  "паспорте нет; альтернативная методика аттестации "
                  "(альфа-счёт, РД 50-373-82) утечкой бы не подавлялась, "
                  "и тогда вывод не работает (см. §19 remarks). Обе "
                  "амплитуды СВОБОДНЫ (по форме спектра, метод 2), ничего "
                  "не зафиксировано на паспорте: ДПР (Pb-214+Bi-214 -- "
                  "линии, которыми активность Ra-226 определяется по "
                  "стандартной практике) = %.2f паспорта, Ra-226 (линия "
                  "186,211 кэВ и прилегающий континуум в окне 100-186 кэВ, "
                  "не только сама линия) = %.2f паспорта. Доля активности "
                  "Ra-226, не проявляющаяся в счёте ДПР сосуда: РАБОЧАЯ "
                  "точка f = %.1f%%, но диапазон по границе окна подгонки "
                  "%.1f-%.1f%% (систематика формы модели, на порядок шире "
                  "формальной статистической ±%.1f%%) -- см. комментарий "
                  "в коде для чисел проверки. НЕ путать с 'коэффициентом "
                  "эманирования' в узком смысле (доля радона, покидающая "
                  "ЗЁРНА матрицы в поровое пространство внутри сосуда, "
                  "измеряется продувкой -- Povinec 2015, ~25%% для "
                  "гранулированного стекла): здесь измерена ДРУГАЯ "
                  "величина -- доля, уходящая ИЗ СЧЁТНОЙ ГЕОМЕТРИИ сосуда "
                  "целиком (негерметичность/диффузия наружу), физически "
                  "сопоставимая с литературой по герметизации Маринелли "
                  "(Syam 2020: 53/30/0%%, Lim 2021: 25/14/0%% по разным "
                  "способам герметизации, §19 remarks) -- рабочая точка "
                  "и диапазон в этих пределах. Линия 186,211 кэВ слабая "
                  "(I=3,565%%), самопоглощение в новой матрице независимо "
                  "не перепроверялось. Контроль на будущее: при "
                  "переизмерении источника ДПР/паспорт должно оставаться "
                  "~1, Ra-226/ДПР ~ 1/(1-f)."
                  % (A_dpr / A_pass, A_ra / A_pass, leak_frac * 100,
                     LEAK_FRAC_WINDOW_LO * 100, LEAK_FRAC_WINDOW_HI * 100,
                     d_leak_frac * 100),
    }
    ...
    with open(out, "w", encoding="utf-8") as f:
        # allow_nan=False -- падать ГРОМКО, если куда-то протёк NaN/inf
        # (найдено аудитом кода 11.08.2026: при вырожденном окне подгонки
        # A_ra/A_dpr <= 0 и leak_frac становится NaN -- Python json.dump
        # по умолчанию пишет литерал NaN, это невалидный JSON, но законный
        # JS-идентификатор, так что деплой не падает, а страница молча
        # печатает "NaN Бк" вместо явной ошибки экспорта).
