...
def main():
    ...
    # Обычный (невзвешенный) МНК по 5 якорям. Три попытки снизить СКО
    # невязки в АБСОЛЮТНЫХ кэВ проверены и отклонены 10.08.2026: взвешенный
    # МНК по 1/d_shift^2 (статистическая погрешность на порядок меньше
    # фактического разброса -- утягивает фит к одному якорю, реальная
    # невязка на 2204 кэВ растёт до 14,9 кэВ), квадратичная поправка
    # (переобучение на 5 точках), явный низкоэнергетичный хвост пика
    # (χ²/ν ухудшается в разы при почти нулевой невязке -- переобучение).
    # Ключевая находка (замечание оператора 10.08.2026: "4,4 кэВ -- это
    # мелочь на высоких [энергиях]", подтверждено численно) -- невязка в
    # АБСОЛЮТНЫХ кэВ растёт с энергией просто потому, что растёт ширина
    # линии (ПШПВ 21 кэВ на 186 кэВ против 121 кэВ на 2204 кэВ): точность
    # деконволюционного определения центроида по своей природе -- доля от
    # ширины пика, не константа в кэВ. В ДОЛЕ ПШПВ невязка у всех пяти
    # линий в одном классе (2-8%, resid_pct_fwhm ниже) -- 2204 кэВ (4,9%)
    # НЕ хуже остальных, у 186/1764 кэВ доля даже выше (7,6%/7,4%). Три
    # отклонённых метода пытались снизить АБСОЛЮТНОЕ число, гоняясь за
    # артефактом единицы измерения, а не за реальной проблемой -- поэтому
    # все либо портили физику, либо переобучались. Простая линейная
    # невзвешенная поправка остаётся верной; d_shift_keV/resid_keV/
    # resid_pct_fwhm сохранены как диагностика.
    ...
    energy_correction = {
        ...
        "anchors": [{"E_nominal": q["E_nominal"], "E_centroid_raw": q["E_centroid"],
                    "d_shift_keV": q.get("d_shift_keV"),
                    "resid_keV": float(r),
                    "fwhm_raw_keV": q.get("fwhm_keV"),
                    "resid_pct_fwhm": (100.0 * abs(float(r)) / q["fwhm_keV"]
                                      if q.get("fwhm_keV") else None)}
                   for q, r in zip(used_anchors, resid)],
    }
    ...
    print("  невязка в доле ПШПВ якоря: %s"
          % ", ".join("%.1f кэВ=%.1f%%" % (a["E_nominal"], a["resid_pct_fwhm"])
                      for a in energy_correction["anchors"]))
    e = corr_a + corr_b * e
