...
(function () {
  ...
  // Подпись и справка величины bg_amplitude -- порт с g1s-th232.js (не
  // буквально 1-в-1: CONT_HINT здесь короче на предложение про "как
  // посчитано", CONT_LAB совпадает дословно; правка аудитора кода
  // 11.08.2026, ra226-remarks.md §22) (находка R44, уже разобрана и
  // исправлена там же): модель обоих
  ...
  function fillCompare() {
    ...
    // источника получен той же методикой. Пятая -- Ra-226 по собственной
    // линии 186,211 кэВ и континууму под ней, до утечки; она выше ДПР
    // в 1/(1-f) раз, то есть на f/(1-f) (не на f -- см. radonNote,
    // radon_dpr_vs_ra_check.caveat).
    var items = [
    ...
    if (D.radon_dpr_vs_ra_check && D.radon_dpr_vs_ra_check.attempted) {
      items.push({ lab: "Ra-226 по 186,211 кэВ (до утечки)",
    ...
    // Принятая версия (решение оператора 10.08.2026 поздно вечером),
    // но строится на ДОПУЩЕНИИ (не подтверждённом документом): источник
    // считается аттестованным ПО ДПР, утечка -- постоянной с 2002 года.
    // Погрешность f публикуется как диапазон по границе окна подгонки
    // (7,7-23,0 %), не как узкая статистическая оценка -- собственная
    // проверка 11.08.2026 (аудит кода + самостоятельный прогон при
    // разных E_FIT_LO/E_FIT_HI) показала систематику модели на порядок
    // шире формальной статистики. Числа живые, из radon_dpr_vs_ra_check
    // (export_ra226_data.py), история -- ra226-remarks.md §17-22.
    if (rn && D.radon_dpr_vs_ra_check && D.radon_dpr_vs_ra_check.attempted) {
      var rdr = D.radon_dpr_vs_ra_check;
      var wLo = rdr.leak_fraction_window_range
        ? rdr.leak_fraction_window_range[0] * 100 : null;
      var wHi = rdr.leak_fraction_window_range
        ? rdr.leak_fraction_window_range[1] * 100 : null;
      rn.innerHTML = "<b>Утечка радона (Rn-222, T½=3,82 сут) — принятая "
        + "версия, на допущении.</b> Две независимые свободные амплитуды "
        + "по форме спектра (метод 2), ничего не зафиксировано: ДПР "
        ...
        + "× паспорта); Ra-226 (линия 186,211 кэВ и континуум под ней) = "
        + cnt(rdr.A_Ra226_Bq) + " ± " + cnt(rdr.dA_Ra226_Bq) + " Бк ("
        + num(rdr.A_Ra226_over_passport, 2) + "× паспорта). "
        + "Допущение: источник аттестован по ДПР, утечка постоянна с "
        + "2002 года — не подтверждено документом; при альтернативной "
        + "методике аттестации (альфа-счёт) вывод не работает. Если "
        + "допущение верно — активность Ra-226 в источнике по линии "
        + "186,211 кэВ (до утечки): <b>" + cnt(rdr.A_Ra226_Bq) + " ± "
        + cnt(rdr.dA_Ra226_Bq) + " Бк</b>; доля активности, не "
        + "проявляющаяся в счёте ДПР сосуда: рабочая точка <b>"
        + num(rdr.leak_fraction * 100, 1) + " %</b>"
        + (wLo !== null ? ", диапазон по границе окна подгонки <b>"
           + num(wLo, 1) + "–" + num(wHi, 1) + " %</b>" : "")
        + " — на порядок шире формальной статистической ±"
        + num(rdr.d_leak_fraction * 100, 1) + " %. Это НЕ коэффициент "
        + "эманирования в узком смысле (доля радона, покидающая зёрна "
        + "матрицы внутрь сосуда, ~25 % для гранулированного стекла, "
        + "Povinec 2015) — измерена другая величина: доля, уходящая из "
        + "счётной геометрии сосуда целиком, сопоставимая с литературой "
        + "по герметизации маринелли (Syam 2020: 53/30/0 %, Lim 2021: "
        + "25/14/0 % по способам герметизации). Линия 186,211 кэВ слабая "
        + "(I=3,565 %), самопоглощение в новой матрице независимо не "
        + "перепроверялось. Контроль при будущем переизмерении: "
        + "ДПР/паспорт должно оставаться ~1, Ra-226/ДПР ~ 1/(1−f).";
    } else if (rn) {
      rn.textContent = "";
    }
  }
