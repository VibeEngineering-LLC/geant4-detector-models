...
(function () {
  ...
  function fillCompare() {
    ...
    // Честный статус (замечания оператора, 10.08.2026 вечером, четыре
    // раунда, каждый следующий уточнял предыдущий):
    // 1) свободный 2-/3-амплитудный фит по 186,211 кэВ даёт "родителя"
    //    ~1,3× паспорта -- похоже на утечку на вид.
    // 2) оператор: гипотеза U-235 ОТКЛОНЕНА ("в нашем КИ этого нет"),
    //    линия 186,211 профессионально не используется для активности.
    // 3) первая попытка решающего теста (закрепить "родителя" на
    //    паспорте) была КОНЦЕПТУАЛЬНО НЕВЕРНОЙ -- оператор поправил:
    //    паспортная активность источника традиционно получена ПО ДПР
    //    (Pb-214/Bi-214) или альфа-счётом, НЕ по линии Ra-226 (см. §19
    //    remarks, sci-search) -- значит закреплять надо ДПР, а не Ra.
    // 4) оператор: "подгоняй ОТДЕЛЬНО активность ДПР и активность радия
    //    по форме спектра и сравнивай" -- НИЧЕГО не фиксировать, обе
    //    амплитуды свободны, сравнить друг с другом и с паспортом.
    // Итог: ДПР (Pb214+Bi214) сходится с паспортом (~1,05×), Ra-226 --
    // заметно выше (~1,3×). Направление рассогласования (Ra-226 высок,
    // не ДПР низок) физически НЕ СООТВЕТСТВУЕТ утечке радона (та занизила
    // бы ДПР относительно Ra-226, а не наоборот) -- см. export_ra226_
    // data.py.radon_dpr_vs_ra_check.caveat, ra226-remarks.md §17-20.
    if (rn && D.radon_dpr_vs_ra_check && D.radon_dpr_vs_ra_check.attempted) {
      var rdr = D.radon_dpr_vs_ra_check;
      rn.innerHTML = "<b>ДПР против Ra-226 (Rn-222, T½=3,82 сут) — ⚠️ "
        + "посчитано, НЕ подтверждено.</b> Две НЕЗАВИСИМЫЕ свободные "
        + "амплитуды по форме спектра (метод 2), ничего не зафиксировано: "
        + "ДПР (Pb-214+Bi-214 — линии, реально используемые для активности "
        + "Ra-226 по стандартной практике, см. ниже) = "
        + cnt(rdr.A_DPR_Bq) + " ± " + cnt(rdr.dA_DPR_Bq) + " Бк ("
        + num(rdr.A_DPR_over_passport, 2) + "× паспорта); Ra-226 (линия "
        + "186,211 кэВ, + Rn-222/Po-218/Po-214 — гамма практически нет) = "
        + cnt(rdr.A_Ra226_Bq) + " ± " + cnt(rdr.dA_Ra226_Bq) + " Бк ("
        + num(rdr.A_Ra226_over_passport, 2) + "× паспорта). ДПР сходится "
        + "с паспортом, Ra-226 заметно выше. Паспортная активность "
        + "традиционно получена ПО ДПР или альфа-счётом, а не по линии "
        + "Ra-226 (та профессионально не используется для активности — "
        + "самопоглощение, неразделимое наложение на линию U-235 "
        + "185,715 кэВ, разница 0,5 кэВ, не берёт даже HPGe хорошего "
        + "разрешения — 5 независимых источников, sci-search 10.08.2026); "
        + "гипотеза примеси U-235 как причины избытка проверена и "
        + "ОТКЛОНЕНА отдельно (паспорт контрольного источника U-235 не "
        + "декларирует). Направление расхождения (Ra-226 высок, не ДПР "
        + "низок) физически НЕ СООТВЕТСТВУЕТ утечке радона — та занизила "
        + "бы ДПР относительно Ra-226, а расходится ровно наоборот. "
        + "Вероятнее — систематика модели/самопоглощения именно на "
        + "186,211 кэВ в новой, ещё не перепроверенной матрице (ОИСН-06, "
        + "первый прогон), не проверено независимым способом. Утечка "
        + "радона из насыпных/гранулированных источников — реальный, "
        + "задокументированный механизм (~25% эманирования для "
        + "гранулированной стеклянной матрицы, Povinec 2015), но прямых "
        + "данных по эпоксидной крошке нет и для ЭТОГО измерения "
        + "предсказательной силы не имеет — держится на будущее "
        + "(переизмерение источника через годы покажет, не просело ли "
        + "отношение ДПР/паспорт).";
    } else if (rn) {
      rn.textContent = "";
    }
  }
