...
(function () {
  ...
  function fillCompare() {
    ...
    // ДПР, как и паспорт источника (аттестация тоже по ДПР). Пятая --
    // Ra-226 по собственной линии 186,211 кэВ, до утечки радона; она
    // выше остальных ровно на коэффициент эманирования (см. radonNote).
    var items = [
    ...
    // источник аттестован ПО ДПР -- при постоянной утечке радона
    // аттестация видела ту же подавленную ДПР-активность, что и это
    // измерение, поэтому ДПР~паспорт и избыток Ra-226 над паспортом --
    // ровно предсказание утечки, не противоречие ей. Истинная
    // активность Ra-226 -- по линии 186,211 кэВ; коэффициент
    // эманирования f = 1 - ДПР/Ra-226. Числа живые, из
    // radon_dpr_vs_ra_check (export_ra226_data.py), история --
    // ra226-remarks.md §17-21.
    if (rn && D.radon_dpr_vs_ra_check && D.radon_dpr_vs_ra_check.attempted) {
      ...
      rn.innerHTML = "<b>Утечка радона (Rn-222, T½=3,82 сут) — принятая "
        + "версия.</b> Две независимые свободные амплитуды по форме "
        + "спектра (метод 2), ничего не зафиксировано: ДПР (Pb-214+Bi-214 "
        + "— линии, которыми активность Ra-226 определяется по стандартной "
        + "практике) = " + cnt(rdr.A_DPR_Bq) + " ± " + cnt(rdr.dA_DPR_Bq)
        + " Бк (" + num(rdr.A_DPR_over_passport, 2) + "× паспорта); "
        + "Ra-226 (линия 186,211 кэВ) = " + cnt(rdr.A_Ra226_Bq) + " ± "
        + cnt(rdr.dA_Ra226_Bq) + " Бк ("
        + num(rdr.A_Ra226_over_passport, 2) + "× паспорта). Источник "
        + "аттестован по ДПР — при постоянной утечке радона аттестация "
        + "видела ту же подавленную ДПР-активность, что и это измерение, "
        + "поэтому согласие ДПР с паспортом и избыток Ra-226 над "
        + "паспортом — ровно та картина, которую утечка предсказывает. "
        + "Активность Ra-226 в источнике по линии 186,211 кэВ (до "
        + "утечки): <b>" + cnt(rdr.A_Ra226_Bq) + " ± "
        + cnt(rdr.dA_Ra226_Bq) + " Бк</b>; коэффициент эманирования "
        + "f = 1 − ДПР/Ra-226 = <b>"
        + num(rdr.leak_fraction * 100, 1) + " ± "
        + num(rdr.d_leak_fraction * 100, 1) + " %</b> — по порядку "
        + "согласуется с литературными значениями для насыпных/"
        + "гранулированных матриц (~25 % для гранулированного стекла, "
        + "Povinec 2015). Оговорки: погрешность f приближённая "
        + "(ковариация совместного фита не учтена); линия 186,211 кэВ "
        + "слабая (I=3,565 %), её самопоглощение в матрице независимо не "
