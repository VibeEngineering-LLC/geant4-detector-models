...
# Коэффициент перехода от плотности потока к H*(10) на 662 кэВ, пЗв·см².
#
# ЗНАЧЕНИЕ НЕ СВЕРЕНО С ПЕРВОИСТОЧНИКОМ. Стоявшее здесь 4,13 подписано
# «ICRP 74 с интерполяцией», но скана таблицы A.21 под рукой нет, а
# соседние узлы таблицы по памяти дают на 0,6 и 0,8 МэВ около 3,05 и 3,86,
# откуда линейная интерполяция на 0,662 МэВ даёт примерно 3,3. Расхождение
# в четверть — это не мелочь: вывод о паспортных 30 имп/с зависит от него
# прямо.
#
# Поэтому здесь НЕ выбирается «правильное» число по памяти. Считаются обе
# версии, и обе выводятся: пока таблица не сверена, вывод обязан быть
# устойчивым к выбору, а если не устойчив — это и надо видеть. Тот же
# приём, что с двумя колонками МИА у Гамма-1С.
H10_VARIANTS = ((4.13, "как стояло, подписано ICRP 74"),
                (3.30, "интерполяция по памяти, ТРЕБУЕТ СКАНА A.21"))
H10_662 = H10_VARIANTS[0][0]
...
def main():
    ...
    print("h*(10) на 662 кэВ НЕ СВЕРЕН со сканом ICRP 74 табл. A.21;")
    for h, why in H10_VARIANTS:
        print("   %.2f пЗв·см² (%s) -> поток %.1f см⁻²·с⁻¹, счёт x%.3f"
              % (h, why, 1e6 / 3600.0 / h, H10_662 / h))
