...
# Дефолт — шаблоны на 1e8 событий (в 10 раз больше первых 1e7). Переключение
# через env нужно, чтобы прямо сравнить: если ошибки амплитуд от роста
# статистики НЕ падают, значит они не от шума шаблонов, а от вырожденности.
TEMPLATE_FMT = os.environ.get("G4MODELS_TEMPLATE_FMT",
                              "rc103_field_m1_%s_1e8.csv")
