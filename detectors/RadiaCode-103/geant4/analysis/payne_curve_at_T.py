    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                continue
    # Отдельная неразбираемая строка законна (заголовки, мусор в хвосте), но
    # ПУСТОЙ набор означает, что не разобралось НИЧЕГО: другой разделитель,
    # другая кодировка, не тот файл. Прежде это доходило до `arr[:, 0]` и
    # падало невнятным IndexError уже вне функции, теряя причину
    # (скан тихих отказов 07.09.2026, класс W-067).
    if not data:
        raise SystemExit(
            "Не разобрано ни одной строки данных в %s — проверьте формат "
            "файла (ожидаются числовые колонки E, выход)" % path)
    arr = np.array(data)
