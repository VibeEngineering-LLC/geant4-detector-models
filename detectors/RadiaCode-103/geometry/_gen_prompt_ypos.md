Задача: применить точечную правку к файлу main.cc (Geant4, C++) по спеке ниже.
Верни ПОЛНЫЙ текст обновлённого файла main.cc целиком — без markdown fences, без пояснений
до/после, только валидный C++ код от первой строки до последней. Изменения делать СТРОГО
по спеке: только добавления, описанные в спеке (6 пунктов). Всё остальное в файле —
дословно как в исходнике, ничего не переформатировать, не менять комментарии, не менять
порядок объявлений, не "улучшать" стиль.
...
=== ТЕКУЩИЙ main.cc (применить правку к этому тексту) ===
...
public:
  ...
  void Fill(double edepKeV, double eprim, int chan = -1) {
    fSumEprim += eprim;
    if (edepKeV <= 0) return;
    ++fWithSignal;
    int b = static_cast<int>(edepKeV / kBinKeV);
...
public:
  ...
  void EndOfEventAction(const G4Event* e) override {
    ...
    fRun->Fill(fEdep / keV, ep, Channel());
  }
};
...
Верни ПОЛНЫЙ обновлённый main.cc: