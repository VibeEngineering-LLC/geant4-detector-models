...
└── verify/
    └── RC110_align_check.png    ← генерируется
```
...
⚠ **`geant4/scripts/verify_align.py` — чужой файл (исправлено 2026-08-26):**
под этим именем и путём лежит скрипт ДРУГОГО детектора (докстрока «Сверка
геометрии AtomSpectra PRO», импорт `atomspectra_pro_coords`, пишет
`AtomSpectraPro_align_check.png`) — для RC-110 нерабочий, не удалять
бездумно (возможно, чужая рабочая копия, не моя зона §12). **Настоящий,
рабочий `verify_align.py` для RC-110 лежит ОДНИМ УРОВНЕМ ВЫШЕ** — в корне
референсной папки (`<референсная RC-110>\verify_align.py`, НЕ в `geant4/`).
Прежняя запись «скрипта уже нет» была ошибочной (проверялся только
`geant4/scripts/`, не корень) — проверено чтением файла 2026-08-26:
константы (`CRYSTAL_C=-50.25`, `CAPSULE_C=-49.9`, PCB/LiPo/CASE) захардкожены,
не читаются из GDML, но численно совпадают с текущей геометрией — сегодняшняя
правка p_node/p_capsule намеренно сохранила мировые координаты неизменными.
`verify/RC110_align_check.png` актуален по крупным узлам (корпус/плата/
LiPo/кристалл/SiPM), но **не рисует ESR вовсе** (ни старую 5-плитную
версию, ни сегодняшнюю 6-плитную с вырезом) — это неполнота охвата
картинки, не ошибка в ней.
...
- Для загрузки GDML: пересобрать Geant4 с `GEANT4_USE_GDML=ON` или перенести геометрию в C++.
