...
**Обновлено:** 2026-08-25 — добавлены плата, LiPo, дисплей, USB; файлы разложены по подпапкам.  
...
| ESR 3M | 65 µm, 5 граней | вокруг кристалла | `ESR_foil` |
...
⚠ **`verify_align.py` в репозиторий НЕ включён:** в референсной папке на
2026-08-26 под этим именем лежит скрипт ДРУГОГО детектора (докстрока
«Сверка геометрии AtomSpectra PRO», импорт `atomspectra_pro_coords`,
пишет `AtomSpectraPro_align_check.png`, ноль упоминаний RC-110) — для
RC-110 он нерабочий. `verify/RC110_align_check.png` сгенерирован прежней,
корректной версией скрипта, которой в папке уже нет.
...
- ESR optical skin на −Z в C++ может потребовать отдельного thin skin (в GDML — 5 slab, без окна −Z в ESR_pz — см. Blender).
