...
// максимум размытой суммы в окне 300–400 кэВ — из данных страницы (model_sum_L72.json), не набран руками
...
function drawChart() {
  ...
  // полупрозрачная полоса: предсказание Geant4 по 4 литературным kB (от максимума Gd+Sm при 0,0121 до максимума при 0,0065)
  if (BAND && BAND[1] >= xlo && BAND[0] <= xhi) {
    const a = X(Math.max(BAND[0], xlo)), b = X(Math.min(BAND[1], xhi));
    g.fillStyle = css("--acc-blue", "#3b6fb6");
    g.globalAlpha = 0.12;
  ...
  MARKERS.forEach((m, k) => {
    ...
    let left = m.left ?? (k === 0), yy = y0 + 4 + (m.row || 0) * 14;   // row — отдельная строка подписи (Belli между двумя другими)
    if (left && x - 4 - w < x0 + 2) { left = false; yy = y0 + 18 + (m.row || 0) * 14; }
    if (!left && x + 4 + w > x1 - 2) { left = true; yy = y0 + 18 + (m.row || 0) * 14; }
