  ...
  function fillCompare() {
    ...
    if (rn && D.radon_fixed_parent_check && D.radon_fixed_parent_check.attempted) {
      ...
      rn.innerHTML = "<b>Утечка радона (Rn-222, T½=3,82 сут) — ⚠️ два "
        ...
        + "<u>Внешняя проверка (sci-search, 10.08.2026)</u>: активность "
        + "Ra-226 по гамма-спектру на практике определяют через ДПР "
        + "(Pb-214/Bi-214), не через 186,211 кэВ — линия неразделимо "
        + "накладывается на U-235 185,715 кэВ (разница 0,5 кэВ, не берёт "
        + "даже HPGe хорошего разрешения), 5 независимых источников. "
        + "Утечка радона из насыпных/гранулированных источников — реальный "
        + "механизм (~25% эманирования измерено для гранулированной "
        + "стеклянной матрицы, Povinec 2015), но прямых данных по "
        + "эпоксидной крошке конкретно нет. Итог: гипотеза (б) "
        + "предпочтительнее для ЭТОГО измерения, гипотеза (а) физически "
        + "реальна как явление — держится на будущее (переизмерение через "
        + "годы покажет, просело ли отношение дочерние/родитель).";
    } else if (rn) {
      rn.textContent = "";
    }
  }
