/* Разложение спектра Ra-226 в маринелли — метод 1 (МК-шаблоны) и метод 2.
   Данные — window.RA226 (export_ra226_data.py, пересчёт 16.09.2026 на
   шаблонах g1s_npsm). Короче g1s-th232.js: нет варианта "cs" по
...
(function () {
  ...
  function buildLegend(mode) {
    ...
    D.nuclides.forEach(function (nd) {
      ...
      chip.className = "chip";
      if (inert) { chip.title = nd.key === "XRAY"
        ? "К-рентген дочерних отдельно не выделяется — входит в шаблоны своих нуклидов"
  ...
  // Подпись величины bg_amplitude (пересчёт 16.09.2026): в критериях A2/A2V
  // измеренный фон той же геометрии, приведённый по живому времени
  // (k = live_s / bg_live_s), вычитается и не подгоняется; bg_amplitude = 1.
  // Прежний свободный второй столбец (находка R44) снят.
  var CONT_LAB = "фон вычтен по живому времени, множитель";
  var CONT_HINT = "измеренный фон той же геометрии приведён по живому "
                + "времени (t_обр / t_фон) и вычтен; свободного множителя "
                + "фона в подгонке нет, значение фиксировано.";
    ...
    el.innerHTML =
      ...
      cell("К-рентген дочерних",
           m1.xray_total_per_branch_pct === null || m1.xray_total_per_branch_pct === undefined
             ? "не выделяется"
             : num(m1.xray_total_per_branch_pct, 3) + " % на распад ветви") +
  ...
  function fillTable1() {
    ...
    var html = "<thead><tr><th>нуклид</th><th class='num'>распадов в МК-шаблоне</th>"
      + "<th class='num'>вклад в модель, отсч./с</th><th class='num'>доля модели</th></tr></thead><tbody>";
    D.nuclides.forEach(function (nd) {
      var v = stk[nd.key];
      var s = 0;
  ...
  function fillCompare() {
    ...
    var pass = D.passport;
    // Первые четыре оценки построены на ПОЛНОМ спектре, где подавляюще
    // доминируют линии ДПР (Pb-214/Bi-214) -- то есть меряют активность
    // ДПР; предполагается (не подтверждено документом), что паспорт
    // источника получен той же методикой. Пятая -- амплитуда группы Ra-226
    // (линия 186,211 кэВ и континуум под ней); спектром она не определяется
    // (пик обратного рассеяния Bi-214 609 кэВ, дыра 26), утечка из неё не
    // выводится (radonNote, radon_dpr_vs_ra_check.caveat, пересчёт 16.09.2026).
    var items = [
      { lab: "паспорт (по ДПР)", A: pass.A_Bq, dA: pass.dA_Bq, col: "#6a6558" },
      { lab: "метод 1 (МК-шаблон цепочки)", A: D.method1.A_Bq, dA: D.method1.dA_Bq, col: "#2c6e2c" },
    ...
    if (D.radon_dpr_vs_ra_check && D.radon_dpr_vs_ra_check.attempted) {
      items.push({ lab: "группа Ra-226, 186,211 кэВ", note: " (не определяется)",
                   A: D.radon_dpr_vs_ra_check.A_Ra226_Bq,
                   dA: D.radon_dpr_vs_ra_check.dA_Ra226_Bq, col: "#8a2be2" });
    }
    ...
    if (tbl) {
      ...
      items.forEach(function (it) {
        ...
        html += "<tr><td>" + it.lab + (it.note || "") + "</td><td>" + cnt(it.A) + " ± " + cnt(it.dA) +
          "</td><td>" + num(r, 3) + "</td></tr>";
      });
      html += "</tbody></table>";
      tbl.innerHTML = html;
    ...
    // Пересчёт 16.09.2026 (CFG1-2026-09-16-ra226-page.md, В5 = У-Б): прежняя
    // «принятая версия» утечки радона (10.08.2026) снята. На комплекте
    // поверки 2024 того же источника группа Ra-226 выше паспорта — артефакт
    // формы модели у пика обратного рассеяния (дыра 26 METHOD.md), а не
    // утечка; амплитуда линии 186,211 кэВ спектром не определяется
    // (RESULT-2026-09-14-kit2024-fit.md §4 п.1). Числа живые, из
    // radon_dpr_vs_ra_check (export_ra226_data.py); доля утечки не выводится.
    if (rn && D.radon_dpr_vs_ra_check && D.radon_dpr_vs_ra_check.attempted) {
      var rdr = D.radon_dpr_vs_ra_check;
      rn.innerHTML = "<b>Утечка радона (Rn-222, T½=3,82 сут) этим спектром "
        + "не определяется.</b> Разрез метода 2 на две свободные амплитуды "
        + "по форме спектра: ДПР "
        + "(Pb-214+Bi-214 — линии, которыми активность Ra-226 определяется "
        + "по стандартной практике) = " + cnt(rdr.A_DPR_Bq) + " ± "
        + cnt(rdr.dA_DPR_Bq) + " Бк (" + num(rdr.A_DPR_over_passport, 2)
        + "× паспорта); группа Ra-226 (линия 186,211 кэВ и континуум под ней) = "
        + cnt(rdr.A_Ra226_Bq) + " ± " + cnt(rdr.dA_Ra226_Bq) + " Бк ("
        + num(rdr.A_Ra226_over_passport, 2) + "× паспорта). "
        + "Амплитуда группы Ra-226 спектром не определяется: линия 186,211 кэВ "
        + "(I=3,565 %) лежит на пике обратного рассеяния линии Bi-214 609 кэВ "
        + "(около 180 кэВ), где форма модели Гамма-1С с измерением не "
        + "согласована (дыра 26 реестра метода). Поэтому активность Ra-226 "
        + "определяется по Rn-цепочке — методом 1 (одна амплитуда на цепочку, "
        + "счёт дают линии Pb-214/Bi-214): <b>" + cnt(D.method1.A_Bq) + " ± "
        + cnt(D.method1.dA_Bq) + " Бк</b>; отношение группы Ra-226 к ДПР в разрезе <b>"
        + num(rdr.ratio_Ra226_to_DPR, 2) + "</b> — систематика формы модели, "
        + "а не утечка радона, и доля утечки <b>не выводится</b>. На комплекте "
        + "поверки 2024 того же источника принудительное равновесие Ra = Rn "
        + "ухудшает χ² лишь на 6–9 % — разделить родителя и дочерние по форме "
        + "спектра нельзя. Прежняя редакция страницы публиковала долю утечки "
        + "как принятую версию; действующим методом она не подтверждается.";
    } else if (rn) {
      rn.textContent = "";
    }
  }
  ...
  function buildCal() {
    ...
    if (tbl && !tbl.dataset.built) {
      ...
      if (m.energy_correction && m.energy_correction.applied) {
        var ecr = m.energy_correction;
        var chList = (ecr.sample_anchor_channels || []).map(function(ch, i){
          return "канал " + num(ch, 0) + " → " +
            num((ecr.sample_anchor_kev || [])[i], 2) + " кэВ";
        }).join(", ");
        body += "<tr class='row-dirty'><td>поправка низкоэнергетичной "
          + "шкалы (локальный квадратик по "
          + (ecr.sample_anchor_channels || []).length + " якорям)</td>"
          + "<td class='num' colspan='2'>сшивка с заводской кривой: "
          + "канал " + num(ecr.blend_lo_ch, 0) + "–"
          + num(ecr.blend_hi_ch, 0) + "<br>якоря: " + chList + "</td></tr>";
      }
      body += "</tbody>";
  ...
  function buildFwhmTable() {
    ...
    var head = "<thead><tr><th>линия, кэВ</th>"
      + "<th class='num'>ПШПВ измеренная (поверка 2016), кэВ</th>"
    ...
    body += "<tr class='sum'><td>степенной закон (аппрокс.)</td>"
      ...
      + num(fw.p, 4) + "</sup></td>"
      + "<td class='num'>СКО к трёхчленному закону на 50–3000 кэВ "
      + num(fw.fit_rms_pct, 1) + " %</td>"
  ...
  function drawFwhm() {
    ...
    g.fillStyle = "#c8541c";
    g.fillText("измеренные точки (поверка 2016)", m.l + 10, m.t + 8);
    g.fillStyle = "#0f5aa8";
